...
//    }
     ...
     */
    function set_where_daftar_agama($cari = NULL, $advance_cari = NULL) {
        if (!is_null($cari) && trim($cari) != '') {
            $this->db->like("m.AGAMA", $cari);
        }
    ...
    function get_total_agama($cari = NULL, $advance_cari = NULL) {

        $this->db->from('M_AGAMA m');
     ...
     */
    function get_daftar_master_agama($offset = 0, $cari = NULL, $rowperpage = 10, $advance_cari = NULL) {
        $total_rows = $this->get_total_agama($cari, $advance_cari);
...
//        exit;
        ...
        if ($total_rows > 0) {
            $sql_select = 
                    "`m`.`ID_AGAMA`, " .
                    "UPPER(`m`.`AGAMA`) as AGAMA, " ;

            $this->db->select($sql_select, FALSE); 
//            $this->db->where("IS_ACTIVE",'1');
            $this->set_where_daftar_agama($cari, $advance_cari);

            $query = $this->db->get('M_AGAMA m', $rowperpage, $offset);
...
//                    exit;
    ...
    function get_id_agama_by_nama($agama){
        $this->db->select('ID_AGAMA')
            ->from($this->table)
            ->where("lower(TRIM(AGAMA)) = lower(TRIM('".$agama."'))");
        $query = $this->db->get()->row();
        
        if ( is_object($query) )
            return $query->ID_AGAMA;
        return FALSE;
    }
