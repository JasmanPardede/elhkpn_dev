...
class Mrpemanfaatan extends CI_Model
{

    private $table = 'M_PEMANFAATAN';
...
//        $this->db->where("IS_ACTIVE",'1');
     ...
     */
    function set_where_daftar_pemanfaatan($cari = NULL, $advance_cari = NULL) {
        if (!is_null($cari) && trim($cari) != '') {
            $this->db->like("m.PEMANFAATAN", $cari);
        }
        ...
        if (!is_null($advance_cari) && is_array($advance_cari)) {
            $dictionary_advance_cari = array(
                ...
                "TEXT" => array(
                    "field_to_compare" => "m.PEMANFAATAN",
                    "compare_method" => "like",
                    "ci_slash_string" => TRUE
                ),
            );
    ...
    function get_total_pemanfaatan($cari = NULL, $advance_cari = NULL) {

        $this->db->from('M_PEMANFAATAN m');
        //$this->from_and_join_master_unitkerja();
//        $this->db->where("m.IS_ACTIVE",'1');
        $this->set_where_daftar_pemanfaatan($cari, $advance_cari);        
     ...
     */
    function get_daftar_master_pemanfaatan($offset = 0, $cari = NULL, $rowperpage = 10, $advance_cari = NULL) {
        $total_rows = $this->get_total_pemanfaatan($cari, $advance_cari);
        ...
        if ($total_rows > 0) {
            $sql_select = 
                    "`m`.`ID_PEMANFAATAN`, " .
                    "`m`.`GOLONGAN_HARTA`, " .
                    "UPPER(`m`.`PEMANFAATAN`) as PEMANFAATAN, " ;
...
//            $this->db->where("m.IS_ACTIVE",'1');
            ...
            $query = $this->db->get('M_PEMANFAATAN m', $rowperpage, $offset);
...
//                    exit;
    ...
    function get_nama_pemanfaatan($id) {
        $this->db->select('PEMANFAATAN')
            ->from($this->table)
            ->where('ID_PEMANFAATAN', $id);
        $query = $this->db->get()->row();
        if ( is_object($query) )
            return $query->PEMANFAATAN;
        return NULL;
    }
    ...
    {
        $this->db->where('ID_PEMANFAATAN', $id);
        return $this->db->get($this->table);
    }
    function save($rpemanfaatan)
    {
        $this->db->insert($this->table, $rpemanfaatan);
        return $this->db->insert_id();
    }

    function update($id, $rpemanfaatan)
    {
        $this->db->where('ID_PEMANFAATAN', $id);
        $this->db->update($this->table, $rpemanfaatan);
    }
