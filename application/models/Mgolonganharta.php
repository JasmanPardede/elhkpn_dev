...
class Mgolonganharta extends CI_Model
{

    private $table = 'M_GOLONGAN_HARTA';
    ...
    function list_all() {
        $this->db->order_by('ID_GOLONGAN_HARTA', 'asc');
...
//        $this->db->where("IS_ACTIVE",'1');
     ...
     */
    function set_where_daftar_golonganharta($cari = NULL, $advance_cari = NULL) {
        if (!is_null($cari) && trim($cari) != '') {
            $this->db->like("m.NAMA_GOLONGAN", $cari);
        }
        ...
        if (!is_null($advance_cari) && is_array($advance_cari)) {
            $dictionary_advance_cari = array(
                ...
                "TEXT" => array(
                    "field_to_compare" => "m.NAMA_GOLONGAN",
                    "compare_method" => "like",
                    "ci_slash_string" => TRUE
                ),
            );
    ...
    function get_total_golonganharta($cari = NULL, $advance_cari = NULL) {

        $this->db->from('M_GOLONGAN_HARTA m');
        //$this->from_and_join_master_unitkerja();
//        $this->db->where("m.IS_ACTIVE",'1');
        $this->set_where_daftar_golonganharta($cari, $advance_cari);        
     ...
     */
    function get_daftar_master_golonganharta($offset = 0, $cari = NULL, $rowperpage = 10, $advance_cari = NULL) {
        $total_rows = $this->get_total_golonganharta($cari, $advance_cari);
        ...
        if ($total_rows > 0) {
            $sql_select = 
                    "`m`.`ID_GOLONGAN_HARTA`, " .
                    "UPPER(`m`.`NAMA_GOLONGAN`) as NAMA_GOLONGAN, " ;
...
//            $this->db->where("m.IS_ACTIVE",'1');
            ...
            $query = $this->db->get('M_GOLONGAN_HARTA m', $rowperpage, $offset);
...
//                    exit;
    ...
    function get_nama_golonganharta($id) {
        $this->db->select('NAMA_GOLONGAN')
            ->from($this->table)
            ->where('ID_GOLONGAN_HARTA', $id);
        $query = $this->db->get()->row();
        if ( is_object($query) )
            return $query->NAMA_GOLONGAN;
        return NULL;
    }
    ...
    {
        $this->db->where('ID_GOLONGAN_HARTA', $id);
        return $this->db->get($this->table);
    }
    function save($golonganharta)
    {
        $this->db->insert($this->table, $golonganharta);
        return $this->db->insert_id();
    }

    function update($id, $golonganharta)
    {
        $this->db->where('ID_GOLONGAN_HARTA', $id);
        $this->db->update($this->table, $golonganharta);
    }
