...
//        $this->db->where("IS_ACTIVE",'1');
     ...
     */
    function set_where_daftar_eselon($cari = NULL, $advance_cari = NULL) {
        if (!is_null($cari) && trim($cari) != '') {
            $this->db->like("m.ESELON", $cari);
        }
    ...
    function get_total_eselon($cari = NULL, $advance_cari = NULL) {

        $this->db->from('M_ESELON m');
//        $this->db->where("IS_ACTIVE",'1');
        $this->set_where_daftar_eselon($cari, $advance_cari);        
     ...
     */
    function get_daftar_master_eselon($offset = 0, $cari = NULL, $rowperpage = 10, $advance_cari = NULL) {
        $total_rows = $this->get_total_eselon($cari, $advance_cari);
...
//        exit;
        ...
        if ($total_rows > 0) {
            $sql_select = 
                    "`m`.`ID_ESELON`, " .
                    "`m`.`KODE_ESELON`, " .
                    "UPPER(`m`.`ESELON`) as ESELON, " ;

            $this->db->select($sql_select, FALSE); 
//            $this->db->where("IS_ACTIVE",'1');
            $this->set_where_daftar_eselon($cari, $advance_cari);

            $query = $this->db->get('M_ESELON m', $rowperpage, $offset);
...
//                    exit;
    ...
    function get_nama_eselon($id) {
        $this->db->select('ESELON')
            ->from($this->table)
            ->where('ID_ESELON', $id);
    ...
    function save($eselon)
    {
        $this->db->insert($this->table, $eselon);
        return $this->db->insert_id();
    }

    function update($id, $eselon)
    {
        $this->db->where('ID_ESELON', $id);
        $this->db->update($this->table, $eselon);
    }
