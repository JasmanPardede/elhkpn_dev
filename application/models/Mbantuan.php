...
class Mbantuan extends CI_Model {

    private $table = 'M_BANTUAN';
...
//        $this->db->where("IS_ACTIVE",'1');
     ...
     */
    function set_where_daftar_bantuan($cari = NULL, $advance_cari = NULL) {
        if (!is_null($cari) && trim($cari) != '') {
            $this->db->like("m.NAME_FORM", $cari);
        }
    ...
    function get_total_bantuan($cari = NULL, $advance_cari = NULL) {

        $this->db->from('M_BANTUAN m');
//        $this->db->where("IS_ACTIVE",'1');
        $this->set_where_daftar_bantuan($cari, $advance_cari);        
     ...
     */
    function get_daftar_master_bantuan($offset = 0, $cari = NULL, $rowperpage = 10, $advance_cari = NULL) {
        $total_rows = $this->get_total_bantuan($cari, $advance_cari);
...
//        exit;
        ...
        if ($total_rows > 0) {
            $sql_select = 
                    ...
                    "`m`.`FORM_HELP`, " .
                    "`m`.`FORM_TOOLTIP`, " .                    
                    "UPPER(`m`.`FORM_TITLE`) as FORM_TITLE, " ;

            $this->db->select($sql_select, FALSE); 
//            $this->db->where("IS_ACTIVE",'1');
            $this->set_where_daftar_bantuan($cari, $advance_cari);

            $query = $this->db->get('M_BANTUAN m', $rowperpage, $offset);
...
//                    exit;
    ...
    function get_nama_bantuan($id) {
        $this->db->select('FORM_TITLE')
            ->from($this->table)
            ->where('ID', $id);
        $query = $this->db->get()->row();
        if ( is_object($query) )
            return $query->FORM_TITLE;
        return NULL;
    }
    ...
    function save($bantuan)
    {
        $this->db->insert($this->table, $bantuan);
        return $this->db->insert_id();
    }

    function update($id, $bantuan)
    {
        $this->db->where('ID', $id);
        $this->db->update($this->table, $bantuan);
    }
