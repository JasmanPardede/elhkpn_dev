...
class Mjenispengeluarankas extends CI_Model {

    private $table = 'M_JENIS_PENGELUARAN_KAS';
    ...
    function list_all() {
        $this->db->order_by('ID_JENIS_PENGELUARAN_KAS', 'asc');
...
//        $this->db->where("m.IS_ACTIVE",'1');
     ...
     */
    function set_where_daftar_jenispengeluarankas($cari = NULL, $advance_cari = NULL) {
    ...
    function get_total_jenispengeluarankas($cari = NULL, $advance_cari = NULL) {

        $this->db->from('M_JENIS_PENGELUARAN_KAS m');
        $this->from_and_join_master_jenispengeluarankas();
//        $this->db->where("m.IS_ACTIVE",'1');
        $this->set_where_daftar_jenispengeluarankas($cari, $advance_cari);        
     ...
     */
    function get_daftar_master_jenispengeluarankas($offset = 0, $cari = NULL, $rowperpage = 10, $advance_cari = NULL) {
        $total_rows = $this->get_total_jenispengeluarankas($cari, $advance_cari);
...
//        exit;
        ...
        if ($total_rows > 0) {
            $sql_select = 
                    "`m`.`ID_JENIS_PENGELUARAN_KAS`, " .
                    "`m_golongan_pengeluaran_kas`.`NAMA_GOLONGAN`, " .
...
//            $this->db->where("m.IS_ACTIVE",'1');
            ...
            $query = $this->db->get('M_JENIS_PENGELUARAN_KAS m', $rowperpage, $offset);
...
//                    exit;
    ...
    function from_and_join_master_jenispengeluarankas() {
        $this->db->join("m_golongan_pengeluaran_kas", "`m`.`GOLONGAN` = `m_golongan_pengeluaran_kas`.`ID_GOLONGAN_PENGELUARAN_KAS`", "left");
    }
    ...
    function get_nama_jenispengeluarankas($id) {
        $this->db->select('NAMA')
            ->from($this->table)
            ->where('ID_JENIS_PENGELUARAN_KAS', $id);
    ...
    {
        $this->db->where('ID_JENIS_PENGELUARAN_KAS', $id);
        return $this->db->get($this->table);
    }

    function save($jenispengeluarankas)
    {
        $this->db->insert($this->table, $jenispengeluarankas);
        return $this->db->insert_id();
    }

    function update($id, $jenispengeluarankas)
    {
        $this->db->where('ID_JENIS_PENGELUARAN_KAS', $id);
        $this->db->update($this->table, $jenispengeluarankas);
    }
