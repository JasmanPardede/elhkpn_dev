    ...
    <div class="modal-body row">
        <div class="col-sm-6">
            ...
            <div class="form-group">
                <label>Nomor Rekening / No Nasabah <span class="red-label">*</span></label>
                <input type="text" name="NOMOR_REKENING" id="NOMOR_REKENING" class="form-control input_capital" required value="<?php echo $onAdd ? encrypt_username($harta->NOMOR_REKENING,'d') : ''; ?>"  />
            </div> 
			...
			<div class="form-group form-file">
                ...
                <label class="control-label">Bukti Dokumen/Rekening (pdf/jpg/png/jpeg/tif)</label> <?= FormHelpPopOver('bukti_dokumen_sb'); ?>
            ...
            <div class="form-group">
                <label>Penerbit/Perusahaan <span class="red-label">*</span></label>
            ...
            <div class="form-group">
                <label>Custodian/Sekuritas <span class="red-label">*</span></label>
...
if (OPERA) {
    ...
    $(document).ready(function () {
        ...
        var list_jenis_harta = load_html('eaudit/klarifikasi/get_jenis_harta_with_data/4');
        $('#KODE_JENIS').html(list_jenis_harta);
        
		var list_pasangan_anak = load_html('eaudit/klarifikasi/get_pasangan_anak_by_id/4/'+ID_LHKPN); 
        $('#KET_PASANGAN_ANAK').html(list_pasangan_anak); 
		
// 		var atas_nama_harta = load_html('portal/data_harta/get_atas_nama_harta/4', '<?php echo $harta->ID ?>' ); 
...
// 		}
		...
		// var list_asal_usul = load_html('portal/data_harta/get_asal_usul_with_data_p/4', '{"asal_usul" : [<?php echo $onAdd ? $harta->ASAL_USUL : ''; ?>],  "ID" : <?php echo $harta->ID ?>}' ); 
		// $('#table-asal-usul').html('<tbody>' + list_asal_usul + '</tbody>');

        var list_asal_usul = load_html('eaudit/klarifikasi/get_asal_usul_with_data_p/4', '{"asal_usul" : [<?php echo $onAdd ? $harta->ASAL_USUL : ''; ?>],  "ID" : <?php echo $harta->ID ?>}' );
        $('#table-asal-usul').html('<tbody>' + list_asal_usul + '</tbody>');
        ...
        if (error == 0 && !asal_usul) {
            ...
            if ((($('#status_harta').val() == '3' && $('#is_load_harta').val() == '1') || $('#status_harta').val() == '2' || $('#status_harta').val() == '1') && $('#is_pelepasan_harta').val() !== '1'){
                if ($('#NILAI_PELAPORAN').val() == 0 || $('#NILAI_PELAPORAN').val() == '0'){
                    ...
                    do_submit('#FormHarta', '/eaudit/klarifikasi/do_update_harta_surat_berharga/edit', text, '#myModal');
                }
