 ...
 * @package Views/masterdata/golonganharta
*/
?>
...
<table id="dt_mastergolonganharta" class="table table-striped table-bordered table-hover table-heading no-border-bottom">
    <thead>
        <tr>
            <th width="30">No.</th>
...
//            var key = $(self).parents('td').children('.key').val();
        ...
        var tblMasterGolonganHarta = {
            tableId: 'dt_mastergolonganharta',
            reloadFn: {tableReload: true, tableCollectionName: 'tblMasterGolonganHarta'},
            conf: {
                "bJQueryUI": true,
                "sPaginationType": "full_numbers",
                "bServerSide": true,
                "sAjaxSource": "<?php echo base_url('admin/masterdata/load_data_daftar_master_golonganharta/'); ?>",
                ...
                "aoColumns": [
                    ...
                    {"mDataProp": "NAMA_GOLONGAN", bSearchable: true},
                    {
                        "mDataProp": function (source, type, val) {
                            ...
                            var BtnHapus = '<button type="button" class="btn btn-sm btn-danger btnDelete" onclick="fnDel(this, ' + source.ID_GOLONGAN_HARTA + ');" title="Delete"><i class="fa fa-trash" ></i></button>';
                            var BtnEdit = '<button type="button" class="btn btn-sm btn-success btnEdit" onclick="fnEdit(this, ' + source.ID_GOLONGAN_HARTA + ');" title="Edit"><i class="fa fa-pencil"></i></button>';
                            var BtnKembalikan = '<button style="display:'+stat+'" type="button" class="btn btn-sm btn-success btnEdit" onclick="fnKembalikan(this, ' + source.ID_GOLONGAN_HARTA + ');" title="Kembalikan"><i class="fa fa-repeat"></i></button>';
        ...
        var gtblMasterGolonganHarta;
        var clearPencarian = function () {
            $('#CARI_TEXT').val('');
            reloadTableDoubleTime(gtblMasterGolonganHarta);
        };
        ...
        $(document).ready(function () {
            gtblMasterGolonganHarta = initDtTbl(tblMasterGolonganHarta);
        });
