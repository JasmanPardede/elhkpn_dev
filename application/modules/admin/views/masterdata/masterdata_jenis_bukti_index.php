 ...
 * @package Views/masterdata/jenisbukti
*/
?>
...
<table id="dt_masterjenisbukti" class="table table-striped">
    <thead>
        <tr>
            <th width="30">No.</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?php echo str_ireplace(@$CARI['TEXT'], '<span style="border-bottom: 1px dashed #999;text-decoration: none;">'.@$CARI['TEXT'].'</span>', $item->JENIS_BUKTI);?></td>
            <td width="120" nowrap="">
...
//            var key = $(self).parents('td').children('.key').val();
        ...
        var tblMasterJenisBukti = {
            tableId: 'dt_masterjenisbukti',
            reloadFn: {tableReload: true, tableCollectionName: 'tblMasterJenisBukti'},
            conf: {
                "bJQueryUI": true,
                "sPaginationType": "full_numbers",
                "bServerSide": true,
                "sAjaxSource": "<?php echo base_url('admin/masterdata/load_data_daftar_master_jenisbukti/'); ?>",
                ...
                "aoColumns": [
                    ...
                    {"mDataProp": "JENIS_BUKTI", bSearchable: true},
                    {
                        "mDataProp": function (source, type, val) {

                            var BtnHapus = '<button type="button" class="btn btn-sm btn-danger btnDelete" onclick="fnDel(this, ' + source.ID_JENIS_BUKTI + ');" title="Delete"><i class="fa fa-trash" ></i></button>';
                            var BtnEdit = '<button type="button" class="btn btn-sm btn-success btnEdit" onclick="fnEdit(this, ' + source.ID_JENIS_BUKTI + ');" title="Edit"><i class="fa fa-pencil"></i></button>';
                            var BtnKembalikan = '<button style="display:'+stat+'" type="button" class="btn btn-sm btn-success btnEdit" onclick="fnKembalikan(this, ' + source.ID_JENIS_BUKTI + ');" title="Kembalikan"><i class="fa fa-repeat"></i></button>';
        ...
        var gtblMasterJenisBukti;
        var clearPencarian = function () {
            $('#CARI_TEXT').val('');
            reloadTableDoubleTime(gtblMasterJenisBukti);
        };
        ...
        $(document).ready(function () {
            gtblMasterJenisBukti = initDtTbl(tblMasterJenisBukti);
        });
