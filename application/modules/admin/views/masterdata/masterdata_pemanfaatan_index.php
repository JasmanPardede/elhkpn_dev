 ...
 * @package Views/masterdata/pemanfaatan
*/
?>
...
<table id="dt_masterpemanfaatan" class="table table-striped">
    <thead>
        <tr>
            <th width="30">No.</th>
            <th>PEMANFAATAN</th>
    ...
    <tbody>
        ...
        <tr>
            <td><?php echo ++$i; ?>.</td>
            <td><?php echo str_ireplace(@$CARI['TEXT'], '<span style="border-bottom: 1px dashed #999;text-decoration: none;">'.@$CARI['TEXT'].'</span>', $item->PEMANFAATAN);?></td>
            <td width="120" nowrap="">
...
//            var key = $(self).parents('td').children('.key').val();
        ...
        var tblMasterPemanfaatan = {
            tableId: 'dt_masterpemanfaatan',
            reloadFn: {tableReload: true, tableCollectionName: 'tblMasterPemanfaatan'},
            conf: {
                "bJQueryUI": true,
                "sPaginationType": "full_numbers",
                "bServerSide": true,
                "sAjaxSource": "<?php echo base_url('admin/masterdata/load_data_daftar_master_pemanfaatan/'); ?>",
                ...
                "aoColumns": [
                    ...
                    {"mDataProp": "PEMANFAATAN", bSearchable: true},
                    {
                        "mDataProp": function (source, type, val) {

                            var BtnHapus = '<button type="button" class="btn btn-sm btn-danger btnDelete" onclick="fnDel(this, ' + source.ID_PEMANFAATAN + ');" title="Delete"><i class="fa fa-trash" ></i></button>';
                            var BtnEdit = '<button type="button" class="btn btn-sm btn-success btnEdit" onclick="fnEdit(this, ' + source.ID_PEMANFAATAN + ');" title="Edit"><i class="fa fa-pencil"></i></button>';
                            var BtnKembalikan = '<button style="display:'+stat+'" type="button" class="btn btn-sm btn-success btnEdit" onclick="fnKembalikan(this, ' + source.ID_PEMANFAATAN + ');" title="Kembalikan"><i class="fa fa-repeat"></i></button>';
        ...
        var gtblMasterPemanfaatan;
        var clearPencarian = function () {
            $('#CARI_TEXT').val('');
            reloadTableDoubleTime(gtblMasterPemanfaatan);
        };
        ...
        $(document).ready(function () {
            gtblMasterPemanfaatan = initDtTbl(tblMasterPemanfaatan);
        });
