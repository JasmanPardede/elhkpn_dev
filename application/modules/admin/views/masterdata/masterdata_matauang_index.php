 ...
 * @package Views/masterdata/matauang
*/
?>
...
<!--  <div class="box-tools">
    ...
    <div class="input-group">
        <input type="text" class="form-control input-sm pull-right" style="width: 150px;" placeholder="Search Nama Mata Uang" name="CARI[TEXT]" value="<?php echo @$CARI['TEXT'];?>" id="CARI_TEXT"/>
...
<table id="dt_mastermatauang" class="table table-striped">
    <thead>
        <tr>
            <th width="30">No.</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <!--d><?php echo str_ireplace(@$CARI['TEXT'], '<span style="border-bottom: 1px dashed #999;text-decoration: none;">'.@$CARI['TEXT'].'</span>', $item->IS_ACTIVE=='1'?'<span style="color:#0099FF;">aktif</span>':'<span style="color:#FF0000;">tidak aktif</span>');?></td-->
            <td width="120" nowrap="">
...
//            var key = $(self).parents('td').children('.key').val();
        ...
        var tblMasterMataUang = {
            tableId: 'dt_mastermatauang',
            reloadFn: {tableReload: true, tableCollectionName: 'tblMasterMataUang'},
            conf: {
                "bJQueryUI": true,
                "sPaginationType": "full_numbers",
                "bServerSide": true,
                "sAjaxSource": "<?php echo base_url('admin/masterdata/load_data_daftar_master_matauang/'); ?>",
                ...
                "aoColumns": [
                    ...
                    {"mDataProp": "SINGKATAN", bSearchable: true},
                    {"mDataProp": "SIMBOL", bSearchable: true},
                    {"mDataProp": "NEGARA", bSearchable: true},
                    {
                        "mDataProp": function (source, type, val) {

                            var BtnHapus = '<button type="button" class="btn btn-sm btn-danger btnDelete" onclick="fnDel(this, ' + source.ID_MATA_UANG + ');" title="Delete"><i class="fa fa-trash" ></i></button>';
                            var BtnEdit = '<button type="button" class="btn btn-sm btn-success btnEdit" onclick="fnEdit(this, ' + source.ID_MATA_UANG + ');" title="Edit"><i class="fa fa-pencil"></i></button>';
                            var BtnKembalikan = '<button style="display:'+stat+'" type="button" class="btn btn-sm btn-success btnEdit" onclick="fnKembalikan(this, ' + source.ID_MATA_UANG + ');" title="Kembalikan"><i class="fa fa-repeat"></i></button>';
        ...
        var gtblMasterMataUang;
        var clearPencarian = function () {
            $('#CARI_TEXT').val('');
            reloadTableDoubleTime(gtblMasterMataUang);
        };
        ...
        $(document).ready(function () {
            gtblMasterMataUang = initDtTbl(tblMasterMataUang);
        });
