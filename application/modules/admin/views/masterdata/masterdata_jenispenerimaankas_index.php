 ...
 * @package Views/masterdata/jenispenerimaankas
*/
?>
...
<table id="dt_masterjenispenerimaankas" class="table table-striped">
    <thead>
        <tr>
            <th width="30">No.</th>
...
//            var key = $(self).parents('td').children('.key').val();
        ...
        var tblMasterJenisPenerimaanKas = {
            tableId: 'dt_masterjenispenerimaankas',
            reloadFn: {tableReload: true, tableCollectionName: 'tblMasterJenisPenerimaanKas'},
            conf: {
                "bJQueryUI": true,
                "sPaginationType": "full_numbers",
                "bServerSide": true,
                "sAjaxSource": "<?php echo base_url('admin/masterdata/load_data_daftar_master_jenispenerimaankas/'); ?>",
                ...
                "aoColumns": [
                    ...
                    {"mDataProp": "NAMA", bSearchable: true},
                    {
                        "mDataProp": function (source, type, val) {
                            ...
                            var BtnHapus = '<button type="button" class="btn btn-sm btn-danger btnDelete" onclick="fnDel(this, ' + source.ID_JENIS_PENERIMAAN_KAS + ');" title="Delete"><i class="fa fa-trash" ></i></button>';
                            var BtnEdit = '<button type="button" class="btn btn-sm btn-success btnEdit" onclick="fnEdit(this, ' + source.ID_JENIS_PENERIMAAN_KAS + ');" title="Edit"><i class="fa fa-pencil"></i></button>';
                            var BtnKembalikan = '<button style="display:'+stat+'" type="button" class="btn btn-sm btn-success btnEdit" onclick="fnKembalikan(this, ' + source.ID_JENIS_PENERIMAAN_KAS + ');" title="Kembalikan"><i class="fa fa-repeat"></i></button>';
        ...
        var gtblMasterJenisPenerimaanKas;
        var clearPencarian = function () {
            $('#CARI_TEXT').val('');
            reloadTableDoubleTime(gtblMasterJenisPenerimaanKas);
        };
        ...
        $(document).ready(function () {
            gtblMasterJenisPenerimaanKas = initDtTbl(tblMasterJenisPenerimaanKas);
        });
