 ...
 * @package Views/masterdata/jenispengeluarankas
*/
?>
...
<!--  <div class="box-tools">
    ...
    <div class="input-group col-sm-push-5">
        <div class="col-sm-3">
        	 <select class="select2" name="CARI[GOLONGAN]" onchange="$('#ajaxFormCari').trigger('submit');" style="width: 350px;" id="CARI_GOLONGAN">
        		<option value="99" <?php echo @$CARI['GOLONGAN']=='99'?'selected':'';?>>-- Golongan --</option>
        		<?php
        		foreach ($reff['golongan'] as $golongan) {
        			$selected = @$CARI['GOLONGAN']==$golongan->ID_GOLONGAN_PENERIMAAN_KAS?'selected':'';
        			echo '<option value="'.$golongan->ID_GOLONGAN_PENERIMAAN_KAS.'" '.$selected.'>'.$golongan->NAMA_GOLONGAN.'</option>';
        		}
        		?>
        	</select>            
        </div>
...
<table id="dt_masterjenispengeluarankas" class="table table-striped">
    <thead>
        <tr>
            <th width="30">No.</th>
            <th>Golongan penerimaan Kas</th>
            <th>Nama</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?php echo str_ireplace(@$CARI['TEXT'], '<span style="border-bottom: 1px dashed #999;text-decoration: none;">'.@$CARI['TEXT'].'</span>', $item->NAMA);?></td>
            <td width="120" nowrap="">
...
//            var key = $(self).parents('td').children('.key').val();
        ...
        var tblMasterJenisPengeluaranKas = {
            tableId: 'dt_masterjenispengeluarankas',
            reloadFn: {tableReload: true, tableCollectionName: 'tblMasterJenisPengeluaranKas'},
            conf: {
                "bJQueryUI": true,
                "sPaginationType": "full_numbers",
                "bServerSide": true,
                "sAjaxSource": "<?php echo base_url('admin/masterdata/load_data_daftar_master_jenispengeluarankas/'); ?>",
                ...
                "aoColumns": [
                    ...
                    {"mDataProp": "NAMA_GOLONGAN", bSearchable: true},
                    {"mDataProp": "NAMA", bSearchable: true},
                    {
                        "mDataProp": function (source, type, val) {

                            var BtnHapus = '<button type="button" class="btn btn-sm btn-danger btnDelete" onclick="fnDel(this, ' + source.ID_JENIS_PENGELUARAN_KAS + ');" title="Delete"><i class="fa fa-trash" ></i></button>';
                            var BtnEdit = '<button type="button" class="btn btn-sm btn-success btnEdit" onclick="fnEdit(this, ' + source.ID_JENIS_PENGELUARAN_KAS + ');" title="Edit"><i class="fa fa-pencil"></i></button>';
                            var BtnKembalikan = '<button style="display:'+stat+'" type="button" class="btn btn-sm btn-success btnEdit" onclick="fnKembalikan(this, ' + source.ID_JENIS_PENGELUARAN_KAS + ');" title="Kembalikan"><i class="fa fa-repeat"></i></button>';
        ...
        var gtblMasterJenisPengeluaranKas;
        var clearPencarian = function () {
            $('#CARI_TEXT').val('');
            reloadTableDoubleTime(gtblMasterJenisPengeluaranKas);
        };
        ...
        $(document).ready(function () {
            gtblMasterJenisPengeluaranKas = initDtTbl(tblMasterJenisPengeluaranKas);
        });
