 ...
 * @package Views/masterdata/provinsi
*/
?>
...
<!--  <div class="box-tools">
    <form method="post" id="ajaxFormCari" action="<?php echo $thisPageUrl;?>">
                    <div class="input-group">
                      <input type="text" class="form-control input-sm pull-right" style="width: 150px;" placeholder="Search Provinsi" name="CARI[TEXT]" value="<?php echo @$CARI['TEXT'];?>" id="CARI_TEXT"/>
...
<table id="dt_masterprovinsi" class="table table-striped">
    <thead>
        <tr>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><small><?php echo str_ireplace(@$CARI['TEXT'], '<span style="border-bottom: 1px dashed #999;text-decoration: none;">'.@$CARI['TEXT'].'</span>', $item->NAME);?></small></td>
            <td width="120" nowrap="">
...
//            var key = $(self).parents('td').children('.key').val();
        ...
        var tblMasterProvinsi = {
            tableId: 'dt_masterprovinsi',
            reloadFn: {tableReload: true, tableCollectionName: 'tblMasterProvinsi'},
            conf: {
                "bJQueryUI": true,
                "sPaginationType": "full_numbers",
                "bServerSide": true,
                "sAjaxSource": "<?php echo base_url('admin/masterdata/load_data_daftar_master_provinsi/'); ?>",
                ...
                "aoColumns": [
                    ...
                    {"mDataProp": "NAME", bSearchable: true},
                    {
                        "mDataProp": function (source, type, val) {
                            ...
                            var BtnHapus = '<button type="button" class="btn btn-sm btn-danger btnDelete" onclick="fnDel(this, ' + source.ID_PROV + ');" title="Delete"><i class="fa fa-trash" ></i></button>';
                            var BtnEdit = '<button type="button" class="btn btn-sm btn-success btnEdit" onclick="fnEdit(this, ' + source.ID_PROV + ');" title="Edit"><i class="fa fa-pencil"></i></button>';
                            var BtnKembalikan = '<button style="display:'+stat+'" type="button" class="btn btn-sm btn-success btnEdit" onclick="fnKembalikan(this, ' + source.ID_PROV + ');" title="Kembalikan"><i class="fa fa-repeat"></i></button>';
        ...
        var gtblMasterProvinsi;
        var clearPencarian = function () {
            $('#CARI_TEXT').val('');
            reloadTableDoubleTime(gtblMasterProvinsi);
        };
        ...
        $(document).ready(function () {
            gtblMasterProvinsi = initDtTbl(tblMasterProvinsi);
        });
