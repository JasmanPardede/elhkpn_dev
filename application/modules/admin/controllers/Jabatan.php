...
class Jabatan extends CI_Controller {
    ...
    public function index($offset = 0) {
        ...
        if ($_POST && $this->input->post('cari', TRUE) != '') {
            ...
            $filter = array(
                    'NAMA_JABATAN'   => $this->input->post('cari', TRUE),
        ...
        $this->items = $this->mjabatan->get_paged_list($this->limit, $this->offset, $filter)->result();
        $this->total_rows = $this->mjabatan->count_all($filter);

        $data = array(
            'bidang' => $this->mglobal->get_data_all('M_BIDANG'),
            'items' => $this->items,
            ...
            'breadcrumb' => call_user_func('ng::genBreadcrumb', array(
                ...
                'Jabatan' => 'index.php/' . strtolower(__CLASS__) . '/' . strtolower(__FUNCTION__),
            )),
