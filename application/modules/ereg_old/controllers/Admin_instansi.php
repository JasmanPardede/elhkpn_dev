...
//        $currpage_found = $curr_page + 1;
    ...
    function saveuser(){
        ...
        if($this->input->post('act', TRUE)=='doinsert'){
			...
			ng::logActivity('Tambah Admin Instansi Untuk, id_user = '.$this->db->insert_id().', username = '.$this->input->post('USERNAME', TRUE));
...
//            }
            ...
            $user['ID_USER']    = $this->input->post('ID_USER', TRUE);
            $this->muser->update($user['ID_USER'], $user);
			ng::logActivity('Hapus Admin Instansi Untuk, id_user = '.$this->input->post('ID_USER', TRUE));
...
//                echo $this->db->last_query();
    ...
    public function getInst($id = NULL)
    {
        if(is_null($id)){
            ...
            $result = $this->mglobal->get_data_all('M_INST_SATKER', null, $where,
                'INST_SATKERKD, INST_NAMA');
