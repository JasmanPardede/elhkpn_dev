...
class Mt_pn_ws_temp extends CI_Model {

    private $table = 't_pn_ws_temp';
    ...
    function save_penambahan_pnwl_ver($id) {
        ...
        $query = "
                    INSERT INTO t_pn (NIK, NIP_NRP, NAMA, EMAIL, IS_ACTIVE, CREATED_TIME, CREATED_BY, CREATED_IP) SELECT
                            NIK,
                            NIP_NRP,
                            NAMA, EMAIL, 1, $time, '$usr', '$ip'
                    FROM
                            t_pn_ws_temp
                    WHERE
                            ID = $id";
    ...
    function save_penambahan_jab_pnwl_ver($id, $id_pn) {
        ...
        $query = "
                INSERT INTO t_pn_jabatan (ID_PN, DESKRIPSI_JABATAN, ID_JABATAN, ESELON, LEMBAGA, NAMA_LEMBAGA, UNIT_KERJA, SUB_UNIT_KERJA, IS_ACTIVE, CREATED_TIME, CREATED_BY, CREATED_IP) SELECT
                        $id_pn,
                        NAMA_JABATAN,
                        (SELECT ID_JABATAN FROM m_jabatan WHERE NAMA_JABATAN LIKE t_pn_ws_temp.NAMA_JABATAN),
                        (SELECT KODE_ESELON FROM m_jabatan WHERE NAMA_JABATAN LIKE t_pn_ws_temp.NAMA_JABATAN),
                        (SELECT INST_SATKERKD FROM m_inst_satker WHERE INST_NAMA = t_pn_ws_temp.NAMA_LEMBAGA), 
                        NAMA_LEMBAGA,
                        NAMA_UNIT_KERJA,
    ...
    function save_penambahan_user_pnwl_ver($id, $pass) {
        ...
        $query = "
                    ...
                    SELECT  NIK, NAMA, NIP_NRP, EMAIL, 0, 5, (SELECT INST_SATKERKD FROM m_inst_satker WHERE INST_NAMA = t_pn_ws_temp.NAMA_LEMBAGA), $time, '$usr', '$ip', '$password'
                    FROM
                        t_pn_ws_temp
                    WHERE
                        ID = $id ";
	...
	`a`.`EMAIL` AS `EMAIL`,
          ...
          TRIM(CONCAT(mjab.NAMA_JABATAN,IFNULL('-',msuk.SUK_NAMA),muk.UK_NAMA)) = TRIM(CONCAT(x.NAMA_JABATAN,IFNULL('-',x.NAMA_SUB_UNIT_KERJA),x.NAMA_UNIT_KERJA)) LIMIT 1) AS CEK 
            FROM
               ...
               JOIN `t_pn_ws_temp` `x` ON((`x`.`NIK` = `a`.`NIK`)))
            WHERE
    ...
    function get_ver_pnwl_tambahan($ins) {
        ...
        if ($is_uk == 1)// cek user unit kerja
            $qry_uk = " AND (SELECT UK_ID FROM m_unit_kerja JOIN m_inst_satker ON INST_SATKERKD = UK_LEMBAGA_ID  WHERE UK_NAMA = t_pn_ws_temp.NAMA_UNIT_KERJA AND INST_NAMA = t_pn_ws_temp.NAMA_LEMBAGA LIMIT 1) = $id_uk";
		...
		WHERE
			TRIM(CONCAT(mjab.NAMA_JABATAN, IFNULL('-',msuk.SUK_NAMA), muk.UK_NAMA)) = TRIM(CONCAT(NAMA_JABATAN, IFNULL('-',NAMA_SUB_UNIT_KERJA), NAMA_UNIT_KERJA))
		LIMIT 1
                ) AS CEK
                 FROM t_pn_ws_temp 
                 WHERE NIK NOT IN(SELECT NIK FROM t_pn WHERE $pqry ) AND IS_PROCESSED = 0 AND $pqry $qry_uk";
        $query = $this->db->query($query);
	...
	`a`.`EMAIL` AS `EMAIL`
            FROM
                    ((((((`t_pn` `a` JOIN `t_pn_jabatan` `b` ON((`a`.`ID_PN` = `b`.`ID_PN`))) JOIN `m_jabatan` `c` ON((`c`.`ID_JABATAN` = `b`.`ID_JABATAN`))) LEFT JOIN `m_sub_unit_kerja` `d` ON((`d`.`SUK_ID` = `c`.`SUK_ID`))) LEFT JOIN `m_unit_kerja` `e` ON((`e`.`UK_ID` = `c`.`UK_ID`))) LEFT JOIN `m_inst_satker` `f` ON((`f`.`INST_SATKERKD` = `e`.`UK_LEMBAGA_ID`))))
            WHERE
                    a.NIK NOT IN (SELECT NIK FROM t_pn_ws_temp WHERE ID_LEMBAGA = $ins) AND ID_STATUS_AKHIR_JABAT = 0 AND IS_CALON = 0 AND IS_DELETED = 0 AND a.IS_ACTIVE = '1' AND b.IS_CURRENT = 1
             $pqry 
            ORDER BY
	`a`.`NAMA`";
        $query = $this->db->query($query);
...
//    For Email
    function get_EmailUserUK($INS) {
        $query = "SELECT DISTINCT  B.EMAIL, UK_ID , B.ID_USER,B.USERNAME FROM t_pn_ws_temp AS A
