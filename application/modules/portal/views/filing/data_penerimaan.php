<form role="form" id="FormPenerimaan">
    <?= FormHelpAccordionEfiling('penerimaan_harta'); ?>
    <div class="box-body">
                ...
                <table class="table table-bordered  table-input">
                    <thead>
                        <tr>
                            <th>NO</th>
                            <th>JENIS PENERIMAAN</th>
                            <th>TOTAL NILAI PENERIMAAN KAS</th>
                        </tr>
                    </thead>
                    <tbody id="block-B">
                    </tbody>    
        ...
        <table class="table table-input">
            ...
            <tr>
                ...
                <td class="text-right">
                    ...
                    <div name="TOTAL_TERBILANG_SUM-ALL" id="TOTAL_TERBILANG_SUM-ALL" style="border-radius:5px;padding:5px;font-style:italic;font-weight:bold;" class="text-right"></div>
                </td>
            </tr>
            <tr>
                <td colspan="4">
                    <button id="btn-save" class="btn btn-sm btn-primary btn-save" >
...
<script type="text/javascript">
    var fMask = function (id, value) {
        $(id).val(value);
        $(id).trigger('mask.maskMoney');
    ...
    $(document).ready(function () {
        ...
        var BLOCK_A = load_html('portal/data_penerimaan/load_penerimaan/A/1/');
        ...
        var BLOCK_B = load_html('portal/data_penerimaan/load_penerimaan/B/2/');
        ...
        var BLOCK_C = load_html('portal/data_penerimaan/load_penerimaan/C/3/');
        ...
        $('#FormPenerimaan').submit(function () {
            $('.input').maskMoney('unmasked')[0];
            $('.input').val() || 0;
            do_submit('#FormPenerimaan', 'portal/data_penerimaan/update', 'Data penerimaan berhasil ditambahkan atau diperbaharui.');
            return false;
        });
        

        $.ajax({
            url: base_url + 'portal/data_penerimaan/load_data/' + ID_LHKPN,
            async: false,
            dataType: 'JSON',
            success: function (data) {
                if (data) {
                    ...
                    /*
                    rollback
                    var total_b3 = total.SUM_B3 || 0;
                    rollback
                    */
                    var total_c = total.SUM_C || 0;
                    var total_semua = total.SUM_ALL || 0;
                    for (i = 0; i < list.length; i++) {
                        var v_pn = list[i].PN || 0;
                        var v_pasangan = list[i].PASANGAN || 0;
                        if (list[i].GROUP_JENIS == 'A') {
                            fMask('#P' + list[i].KODE_JENIS, parseFloat(v_pasangan));
                            $('#TERBILANG_P' + list[i].KODE_JENIS).hide();
//                            $('#P' + list[i].KODE_JENIS).val(parseFloat(v_pasangan));
//                            $('#P' + list[i].KODE_JENIS).trigger('mask.maskMoney');
                        }
                        
                        fMask('#' + list[i].KODE_JENIS, parseFloat(v_pn));
                        $('#TERBILANG_' + list[i].KODE_JENIS).hide();
//                        $('#' + list[i].KODE_JENIS).val(parseFloat(v_pn));
//                        $('#' + list[i].KODE_JENIS).trigger('mask.maskMoney');
                    }
                    /*rollback
                    //tembak ke b3 (Penjualan atau Pelepasan Harta)
                    fMask('#B3', parseFloat(total_b3));
                    rollback
                    */
