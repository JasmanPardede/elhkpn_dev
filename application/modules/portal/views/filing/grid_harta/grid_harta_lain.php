<table id="TableHbergerak"  class="table table-striped table-bordered table-hover table-heading no-border-bottom table-filing">    <thead>        <tr>
...
<!-- alert Penetapan -->
<div class="modal fade" id="alertPenetapanHbl" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        var id_pelepasan_harta = ['4','5','7','8','9','14'];
       ...
       dtHartaBergerakLainnya = $('#TableHbergerak').dataTable({
            "oLanguage": ecDtLang,
            'sPaginationType': 'full_numbers',
            'bServerSide': true,
            'bProcessing': true,
            'sAjaxSource': '<?php echo base_url(); ?>portal/data_harta/grid_harta_bergerak_lain/'+ID_LHKPN,
            "bLengthChange": false,
            "bFilter": true,
            "bSort": false,
//            "bInfo": false,
...
//            "aoColumnDefs": [{ "bVisible": copy, "aTargets": [1] }],
        ...
        // DELETE ACTION
        $("#TableHbergerak tbody").on('click','.delete-action',function(event){
             var id = $(this).attr('id');
             confirm("Apakah anda yakin akan menghapus data ? ", function(){
                 do_delete('portal/data_harta/delete/'+id+'/t_lhkpn_harta_bergerak_lain','Data Harta Bergerak Lainnya berhasil dihapus ');
                 $('#TableHbergerak').DataTable().ajax.reload();
             });
        });

        // PENETEAPAN ACTION
        $("#TableHbergerak tbody").on('click','.penetapan-action',function(event){
             var id = $(this).attr('id');
             confirm("Apakah anda yakin akan melakukan penetapan harta ini ? ", function(){
                 $.ajax({
                    url: base_url + 'portal/data_harta/edit_harta_begerak_lain/' + id,
                    async: false,
                    dataType: 'JSON',
                    success: function (data) {
                        var rs = eval(data);
                        if (rs.result.JUMLAH == 0){
                            notif('Data Harta Bergerak Lain gagal dimutakhirkan dengan status TETAP, dikarenakan jumlah 0 (nol), silakan direview kembali nilai saat pelaporannya dengan fitur PERUBAHAN DATA');
                        }else if (rs.result.NILAI_PELAPORAN == 0){
                            notif('Data Harta Bergerak Lain gagal dimutakhirkan dengan status TETAP, dikarenakan nilai pelaporan 0 (nol), silakan direview kembali nilai saat pelaporannya dengan fitur PERUBAHAN DATA');
                            ...
                            $('#alertPenetapanHbl').modal('toggle');
                        }else{
                              do_delete('portal/data_harta/penetapan/'+id+'/t_lhkpn_harta_bergerak_lain','Data Harta Bergerak Lainnya Sudah Tetap ');
                              $('#TableHbergerak').DataTable().ajax.reload();  
                        }
                    }
                });
         ...
         // PELEPASAN ACTION
        $("#TableHbergerak tbody").on('click','.pelepasan-action',function(event){
             var id = $(this).attr('id');
             $('#FORM_PELEPASAN').bootstrapValidator('resetForm', true);
             $('#FORM_PELEPASAN #JENIS_PELEPASAN_HARTA').select2();
             $('#FORM_PELEPASAN #MAIN_TABLE').val('t_lhkpn_harta_bergerak_lain');
             $('#FORM_PELEPASAN #TABLE').val('t_lhkpn_pelepasan_harta_bergerak_lain');
             $('#FORM_PELEPASAN #ID_HARTA').val(id);
             $('#FORM_PELEPASAN #TABLE_GRID').val('#TableHbergerak');
             $('#FORM_PELEPASAN #NOTIF').val('Data harta bergerak lainnya berhasil dilepas.');
             // KONDISI NILAI
...
//             });
        ...
        // EDIT ACTION
        $("#TableHbergerak tbody").on('click','.edit-action',function(event){
            var id = $(this).attr('id');
            $.ajax({
                 url: base_url +'portal/data_harta/edit_harta_begerak_lain/'+id,
                 async: false,
                 ...
                 success:function(data){
                    ...
                    $('#JUMLAH').val(rs.result.JUMLAH);
                    $('#SATUAN').val(rs.result.SATUAN);
        ...
        // EDIT ACTION ALERT
        $("#alertPenetapanHbl").on('click','.edit-action-alert',function(event){
            $('#alertPenetapanHbl').modal('toggle');
            var id = $(this).attr('id');
            $.ajax({
                 url: base_url +'portal/data_harta/edit_harta_begerak_lain/'+id,
    ...
    function Cancelparent() {
        $('#formAsalUsul').fadeOut('fast', function() {
            $('#FormGerakLain').fadeIn('fast', function() {
                $('#ModalHarta .modal-content').animate({
                    'width': '100%',
                    'margin-left': '0'
                });
