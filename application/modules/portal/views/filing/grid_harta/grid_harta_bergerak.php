<table id="TableMesin"  class="table table-striped table-bordered table-hover table-heading no-border-bottom table-filing">
    <thead>
        <tr>
            <th style="width:10% important;">NO</th>
            <th style="width:10% important;">STATUS</th>
            <th style="width:10% important;">URAIAN</th>
            <th style="width:10% important;">KEPEMILIKAN</th>
...
<div style="overflow:hidden;">
    ...
    <a href="javascript:void(0)" onclick="pindah_tab('#harta_tidak_bergerak','harta_tidak_bergerak')" class="btn btn-warning btn-sm" style="margin-left:5px;">
...
<!-- alert Penetapan -->
<div class="modal fade" id="alertPenetapanHb" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        var id_pelepasan_harta = ['4','5','6','7','8','9','14'];
        ...
        dtMesin = $('#TableMesin').dataTable({
            "oLanguage": ecDtLang,
            'sPaginationType': 'full_numbers',
            'bServerSide': true,
            'bProcessing': true,
            'sAjaxSource': '<?php echo base_url(); ?>portal/data_harta/grid_harta_bergerak/'+ID_LHKPN,
            "bLengthChange": false,
            "bFilter": true,
            "bSort": false,
//            "bInfo": false,
            "bAutoWidth": false,
            'aoColumns': [{sWidth:"1%"},null,{sWidth:"30%"},{sWidth:"25%"},{sWidth:"12%"},{sWidth:"12%"},{sWidth:"12%"}],
...
//            "aoColumnDefs": [{ "bVisible": copy, "aTargets": [1] }],
        ...
        // DELETE ACTION
        $("#TableMesin tbody").on('click','.delete-action',function(event){
             var id = $(this).attr('id');
             confirm("Apakah anda yakin akan menghapus data ? ", function(){
                 do_delete('portal/data_harta/delete/'+id+'/t_lhkpn_harta_bergerak','Data Harta Alat Transportasi/Mesin berhasil dihapus ');
                 $('#TableMesin').DataTable().ajax.reload();
             });
        });


        // PENETAPAN ACTION
        $("#TableMesin tbody").on('click','.penetapan-action',function(event){
             var id = $(this).attr('id');
             confirm("Apakah anda yakin akan melakukan penetapan harta ini ? ", function(){
                 $.ajax({
                    url: base_url + 'portal/data_harta/edit_harta_begerak/' + id,
                    async: false,
                    dataType: 'JSON',
                    success: function (data) {
                        var rs = eval(data);
                        if (rs.result.NILAI_PELAPORAN == 0){
                            notif('Data Harta Bergerak gagal dimutakhirkan dengan status TETAP, dikarenakan nilai pelaporan 0 (nol), silakan direview kembali nilai saat pelaporannya dengan fitur PERUBAHAN DATA');
                            ...
                            $('#alertPenetapanHb').modal('toggle');
                        } else if (rs.result.ATAS_NAMA == null || rs.result.ATAS_NAMA == '') {
                            notif('Data Harta Bergerak gagal dimutakhirkan dengan status TETAP, dikarenakan Atas Nama bernilai kosong atau null, silakan direview kembali');
                            ...
                            notif('Data Harta Bergerak gagal dimutakhirkan dengan status TETAP, dikarenakan Nama Orang Lain / Lainnya bernilai kosong atau null, silakan direview kembali');
                        }else{
                              do_delete('portal/data_harta/penetapan/'+id+'/t_lhkpn_harta_bergerak','Data Harta Tidak Bergerak Sudah Tetap ');
                              $('#TableMesin').DataTable().ajax.reload();  
                        }
                    }
                });
         ...
         // PELEPASAN ACTION
        $("#TableMesin tbody").on('click','.pelepasan-action',function(event){
             var id = $(this).attr('id');
             $('#FORM_PELEPASAN').bootstrapValidator('resetForm', true);
             $('#FORM_PELEPASAN #JENIS_PELEPASAN_HARTA').select2();
             $('#FORM_PELEPASAN #MAIN_TABLE').val('t_lhkpn_harta_bergerak');
             $('#FORM_PELEPASAN #TABLE').val('t_lhkpn_pelepasan_harta_bergerak');
             $('#FORM_PELEPASAN #ID_HARTA').val(id);
             $('#FORM_PELEPASAN #TABLE_GRID').val('#TableMesin');
             $('#FORM_PELEPASAN #NOTIF').val('Data harta Alat Transportasi/Mesin berhasil dilepas.');
             // KONDISI NILAI
...
//            });
        ...
        // EDIT ACTION
        $("#TableMesin tbody").on('click','.edit-action',function(event){
            var id = $(this).attr('id');
            $.ajax({
                 url: base_url +'portal/data_harta/edit_harta_begerak/'+id,
                 async: false,
                 ...
                 success:function(data){
                    ...
                    $('#MEREK').val(rs.result.MEREK);
                    $('#MODEL').val(rs.result.MODEL);
                    $('#TAHUN_PEMBUATAN').val(rs.result.TAHUN_PEMBUATAN);
                    $('#NOPOL_REGISTRASI').val(rs.result.NOPOL_REGISTRASI);
                    $('#JENIS_BUKTI').select2('val',rs.result.ID_JENIS_BUKTI);
                    $('#ATAS_NAMA').select2('val',rs.result.ATAS_NAMA);
                    $('#KET_LAINNYA').val(rs.result.KET_LAINNYA);
                    ...
                    $('#PEMANFAATAN').select2('val',rs.result.ID_PEMANFAATAN); 
        ...
        // EDIT ACTION ALERT
        $("#alertPenetapanHb").on('click','.edit-action-alert',function(event){
            $('#alertPenetapanHb').modal('toggle');
            var id = $(this).attr('id');
            $.ajax({
                 url: base_url +'portal/data_harta/edit_harta_begerak/'+id,
