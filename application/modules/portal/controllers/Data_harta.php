...
class Data_harta extends CI_Controller {
    ...
    function delete($ID, $TABLE) {
        ...
        $check_protect = protectFilling($state_id_pn,$TABLE,$ID);
        if($check_protect){
            $method = __METHOD__;
            $this->load->model('mglobal');
        ...
        $data2 = $this->mglobal->get_by_id($TABLE, $PK, $ID);
        $ID_LHKPN = $data2->ID_LHKPN;
        ng::logActivity("Hapus Data ".$harta.", ID = ".$ID.", ID_LHKPN = ".$data2->ID_LHKPN);
        $this->db->where($PK, $ID);
    ...
    function penetapan($ID, $TABLE) {
        ...
        ng::logActivity("Penetapan Data ".$harta.", ID = ".$ID.", ID_LHKPN = ".$check->ID_LHKPN);
    ...
    function do_update($TABLE, $DATA, $ID = NULL) {
        ...
        if ($ID) {
            ...
            ng::logActivity("Ubah Data ".$harta.", ID = ".$ID.", ID_LHKPN = ".$DATA['ID_LHKPN']);
        } else {
            $result = $this->db->insert($TABLE, $DATA);
            ng::logActivity("Tambah Data ".$harta.", ID = ".$ID.", ID_LHKPN = ".$DATA['ID_LHKPN']);
        }
        if ($result) {
            if ($ID) {
                return $ID;
...
//                $delete .= "<a id='" . $ID . "'  data-nilai=[" . $nilai . "] href='javascript:void(0)' class='btn btn-primary btn-sm penetapan-action' title='Penetapan'><i class='fa fa-link'></i></a>";
    ...
    function get_option_matauang($table, $name, $value) {
        ...
        foreach ($data as $row) {
            if ($row->$value == 1) {
                echo "<option value='" . $row->$value . "' selected>" . strtoupper($row->$name) . "</option>";
            } else {
                echo "<option value='" . $row->$value . "'>" . strtoupper($row->$name) . "</option>";
            }
        }
	...
	function get_asal_usul_with_data_p($gol = NULL) { //ditambah oleh Ferry
        ...
        foreach ($data as $row) {
			...
			foreach($asal_usul as $dom){
				if($row->$value == $dom->ID_ASAL_USUL){
					...
					$keterangan = trim(preg_replace('/\s+/', ' ', $dom->URAIAN_HARTA));
    ...
    function get_asal_usul_with_data() {
...
//            echo "<option " . $selected . " value='" . $row->ID_JENIS_BUKTI . "'>" . strtoupper($row->JENIS_BUKTI) . "</option>";
	...
	function get_pasangan_anak_by_id($table_source = '', $id_lhkpn = '') {
		...
		//	$history = $this->mlhkpnharta->get_id_lhkpn_by_id($table_source,$id);
        ...
        if ($data) {
            echo "<option></option>";
            foreach ($data as $row) {
                echo "<option value='" . $row->ID_KELUARGA . "' selected>" . $row->NAMA . "</option>";
            }
        }
        exit;
    }

	function get_atas_nama_harta($gol = NULL) {
...
//            $checked = !empty($arr_data) && in_array($row->$value, $arr_data) ? "checked=checked" : "";
    ...
    function STATUS_HARTA($id, $is_pelepasan) {
        ...
        $data[3] = '<label class="label" style="background-color: #25CCF7;">Baru</label>';
        if ($is_pelepasan == '1') {
    ...
    function grid_harta_tidak_bergerak($ID_LHKPN) {
        ...
        if ($data) {
            foreach ($data as $list) {
                ...
                $list->ATAS_NAMA = replace_string_null($list->ATAS_NAMA);
                $list->KET_LAINNYA = replace_string_null($list->KET_LAINNYA);
...
//                    $NEGARA = $list->aNEGARA;
                ...
                if(strstr($get_atas_nama, "3")){
                    $list->KET_LAINNYA = trim(preg_replace('/\s+/', ' ', $list->KET_LAINNYA));
                    if (strtolower($list->KET_LAINNYA) == 'null') {
                        $list->KET_LAINNYA = null;
                    }
                    if($atas_nama==''){
                ...
                if ($list->ID_LHKPN == $ID_LHKPN) {
                    $aaData[] = array(
                        ...
                        $list->STATUS_LHKPN == '0' || $list->STATUS_LHKPN == '2' || $list->STATUS_LHKPN == '7' && $list->entry_via == '0' ? $this->action($list->$PK, $TABLE, $list->IS_COPY) : ''
                    );
                }
            }
        }
    ...
    function edit_harta_tidak_begerak($ID) {
        ...
        $result = $this->db->get($TABLE)->row();
        $result->ATAS_NAMA = replace_string_null($result->ATAS_NAMA);
        $result->KET_LAINNYA = replace_string_null($result->KET_LAINNYA);
        $result->PASANGAN_ANAK = replace_string_null($result->PASANGAN_ANAK);
            ...
            foreach ($data as $list) {
                ...
                if ($list->ID_LHKPN == $ID_LHKPN) {
                    $aaData[] = array(
                        ...
                        'Rp. ' . number_rupiah($list->NILAI_PEROLEHAN) . '',
                        'Rp. ' . number_rupiah($list->NILAI_PELAPORAN) . '',
                        $list->STATUS == 0 || $list->STATUS == 2 || $list->STATUS == 7 && $list->entry_via == '0' ? $this->action($list->$PK, $TABLE, $list->IS_COPY) : ''
                    );
                }
            }
        }
...
//        echo $this->db->last_query();exit;
    ...
    function update_harta_bergerak_Lain() {
        ...
        $data = array(
            ...
            'KETERANGAN' => trim(preg_replace('/\s+/', ' ', $this->input->post('KETERANGAN'))),
            'ASAL_USUL' => $ASAL_USUL,
            'MATA_UANG' => 1,
            'STATUS' => $STATUS,
            'IS_PELEPASAN' => $IS_PELEPASAN,
            ...
            foreach ($data as $list) {
                ...
                if ($list->FILE_BUKTI) {
                    ...
                    foreach ($filelist as $key => $tmp_name) {
                        ...
                        $secretId = base64_encode($list->ID);
                        ...
                        //get data from minio
                        $checkFile = linkFromMinio($dir . $tmp_name,null,'t_lhkpn_harta_surat_berharga','ID',$list->ID);
                        if($checkFile){
                            $link = $checkFile;
                           //   $img = $img . " <a class='files' target='_blank' href='" . $checkFile . "'><i class='fa fa-file-text fa-2x'></i></a>";
                            $img = $img . " <a id='".$secretId."' ext='".$ext."' link='".$link."' secretname='".$secretName."'  href='javascript:void(0)' class='files edit-action-document' title='Edit'><i class='fa ".$classIcon." fa-2x'></i></a> ";
                        }else{
				...
				";
                if (strlen($list->NOMOR_REKENING) >= 32){
                    $decrypt_norek = encrypt_username($list->NOMOR_REKENING,'d');
                } else {
                    $decrypt_norek = $list->NOMOR_REKENING;
                }
                if ($list->ID_LHKPN == $ID_LHKPN) {
                    $aaData[] = array(
                        $no_new++,
...
//                        $list->STATUS == 0 || $list->STATUS == 2 ? $this->action($this->m_data_harta_surat_berharga->PK, $this->m_data_harta_surat_berharga->TABLE, $list->IS_COPY) : ''
                        $list->STATUS_LHKPN == 0 || $list->STATUS_LHKPN == 2 || $list->STATUS == 7 && $list->entry_via == '0' ? $this->action($list->ID, $this->m_data_harta_surat_berharga->TABLE, $list->IS_COPY) : ''
                    );
                }
            }
        }
    ...
    function edit_surat_berharga($ID) {
        ...
        if (strlen($result->NOMOR_REKENING) >= 32){
            $decrypt_norek = encrypt_username($result->NOMOR_REKENING,'d');
        } else {
            $decrypt_norek = $result->NOMOR_REKENING;
        }
    ...
    function update_surat_berharga() {
        ...
        if ($ID) {
            ...
            $upload = $this->upload_surat_berharga($ID_LHKPN);
            if ($upload['upload']) {
                if ($temp) {
                    if ($temp->FILE_BUKTI) {
    ...
    function upload_surat_berharga($ID_LHKPN) {
        $result = array();
        ...
        if (!file_exists('uploads/data_suratberharga/' . $folder)) {
            mkdir('uploads/data_suratberharga/' . $folder, 0777, TRUE);
...
//             }
    ...
    function grid_kas($ID_LHKPN) {
        ...
        $iDisplayLength = $this->input->post('iDisplayLength');
        $iDisplayStart = $this->input->post('iDisplayStart');
        $cari = strtoupper($this->input->post('sSearch'));
        $aaData = array();
        $sql_like = "";
        $i = 0;
        $no_new = 1;
        ...
        if ($data) {
            foreach ($data as $list) {
                ...
                $list->ATAS_NAMA_REKENING = replace_string_null($list->ATAS_NAMA_REKENING);
                $list->ATAS_NAMA_LAINNYA = replace_string_null($list->ATAS_NAMA_LAINNYA);
                $list->KETERANGAN = replace_string_null($list->KETERANGAN);
                $list->PASANGAN_ANAK = replace_string_null($list->PASANGAN_ANAK);
                ...
                if ($list->FILE_BUKTI) {
                    ...
                    foreach ($filelist as $key => $tmp_name) {
                        ...
                        //get data from minio
                        $checkFile = linkFromMinio($dir . $tmp_name,null,'t_lhkpn_harta_kas','ID',$list->ID);
                        if($checkFile){
                            $link = $checkFile;
                           //   $img = $img . " <a class='files' target='_blank' href='" . $checkFile . "'><i class='fa fa-file-text fa-2x'></i></a>";
                            $img = $img . " <a id='".$secretId."' ext='".$ext."' link='".$link."' secretname='".$secretName."'  href='javascript:void(0)' class='files edit-action-document' title='Edit'><i class='fa ".$classIcon." fa-2x'></i></a> ";
...
//                 }
                ...
                if ($list->ID_LHKPN == $ID_LHKPN) {
                    $aaData[] = array(
                        ...
                        $list->STATUS_LHKPN == 0 || $list->STATUS_LHKPN == 2 || $list->STATUS == 7 && $list->entry_via == '0' ? $this->action($list->ID, $this->m_data_harta_kas->TABLE, $list->IS_COPY) : ''
                    );
                }
            }
        }
    ...
    function edit_kas($ID) {
        ...
        if (strlen($result->NAMA_BANK) >= 32){
            $decrypt_namabank = encrypt_username($result->NAMA_BANK,'d');
        } else {
            $decrypt_namabank = $result->NAMA_BANK;
        }
        ...
        $result->ATAS_NAMA_REKENING = replace_string_null($result->ATAS_NAMA_REKENING);
        $result->ATAS_NAMA_LAINNYA = replace_string_null($result->ATAS_NAMA_LAINNYA);
        $result->KETERANGAN = replace_string_null($result->KETERANGAN);
        $result->PASANGAN_ANAK = replace_string_null($result->PASANGAN_ANAK);
        
        $data = array(
            'result' => $result,
            'nama_bank' => $decrypt_namabank,
            'norek' => $decrypt_norek,
            'asal_usul' => $asal_usul
        );
        header('Content-Type: application/json');
    ...
    function update_kas() {
        ...
        $ekuivalen = $this->input->post('NILAI_EQUIVALEN');
        $kurs = $this->input->post('NIlAI_KURS');

        //// protect nilai kurs ////
        if($kurs <= 0 or empty($kurs)){
            echo "7"; exit();
        }

        //// protect nilai ekuivalen ////
        if($ekuivalen <= 0 or empty($ekuivalen)){
            echo "5"; exit();
        }
        ...
        if ($ID) {
            $this->db->where($PK, $ID);
            $temp = $this->db->get($TABLE)->row();
            $upload = $this->upload_kas($ID_LHKPN);
            if ($upload['upload']) {
                if ($temp) {
                    if ($temp->FILE_BUKTI) {
                        unlink($temp->FILE_BUKTI);
...
//            'KETERANGAN' => $this->input->post('KETERANGAN'),
            'NILAI_KURS' => intval(str_replace('.', '', $this->input->post('NIlAI_KURS'))),
    ...
    function upload_kas($ID_LHKPN) {
        ...
        if (!file_exists('uploads/data_kas/' . $folder)) {
            mkdir('uploads/data_kas/' . $folder, 0777, TRUE);
            ...
            foreach ($data as $list) {
                ...
                if ($list->FILE_BUKTI) {
                    ...
                    foreach ($filelist as $key => $tmp_name) {
                        ...
                        //get data from minio
                        $checkFile = linkFromMinio($dir . $tmp_name,null,'t_lhkpn_harta_lainnya','ID',$list->ID);
                        if($checkFile){
                            $link = $checkFile;
                           //   $img = $img . " <a class='files' target='_blank' href='" . $checkFile . "'><i class='fa fa-file-text fa-2x'></i></a>";
                            $img = $img . " <a id='".$secretId."' ext='".$ext."' link='".$link."' secretname='".$secretName."'  href='javascript:void(0)' class='files edit-action-document' title='Edit'><i class='fa ".$classIcon." fa-2x'></i></a> ";
...
//                 }
    ...
    function update_harta_Lainnya() {
        ...
        if ($ID) {
            $this->db->where($PK, $ID);
            $temp = $this->db->get($TABLE)->row();
            $upload = $this->upload_harta_lainnya($ID_LHKPN);
            if ($upload['upload']) {
                if ($temp) {
                    if ($temp->FILE_BUKTI) {
                        unlink($temp->FILE_BUKTI);
    ...
    function upload_harta_lainnya($ID_LHKPN) {
        $result = array();
        // $folder = $this->encrypt($this->session->userdata('USERNAME'), 'e');
        $folder = $this->session->userdata('USERNAME');
        $folder = $folder.'/'.$ID_LHKPN;
        if (!file_exists('uploads/data_hartalainnya/' . $folder)) {
            mkdir('uploads/data_hartalainnya/' . $folder, 0777, TRUE);
...
//         }
    ...
    private function __set_join_grid_hutang($TABLE, $PK) {
        $this->db->join('m_jenis_hutang', 'm_jenis_hutang.ID_JENIS_HUTANG = ' . $TABLE . '.KODE_JENIS', 'left');
//        $this->db->join('t_lhkpn', 't_lhkpn.ID_LHKPN = ' . $TABLE . '.ID_LHKPN');
        $this->db->join('t_lhkpn', "t_lhkpn.ID_LHKPN = " . $TABLE . ".ID_LHKPN and ID_PN = '" . $this->session->userdata('ID_PN') . "'");
            ...
            foreach ($data as $list) {
                ...
                if ($list->ID_LHKPN == $ID_LHKPN) {
                    $aaData[] = array(
                        ...
                        $list->STATUS == 0 || $list->STATUS == 2 || $list->STATUS == 7 && $list->entry_via == '0' ? $this->action($list->$PK, $TABLE, '0') : ''
                    );
                }
            }
        }
...
//            'IS_LOAD' => '0',
        ...
        if ($ID) {
            $this->db->where($PK, $ID);
            $result = $this->db->update($TABLE, $data);
            ng::logActivity("Ubah Data Hutang, ID = ".$ID.", ID_LHKPN = ".$data['ID_LHKPN']);
        } else {
            $this->db->insert($TABLE, $data);
            $result = $this->db->insert_id();
            ng::logActivity("Tambah Data Hutang, ID = ".$result.", ID_LHKPN = ".$data['ID_LHKPN']);
        }
        if ($result) {
            echo "1";
        } else {
...
//        return $output;
    ...
    function edit_document_surat_berharga($dir, $tmp) {
    ...
    function delete_file_pendukung($tipe){
        $dirfile = base64_decode($_POST['secretdir']);
        $namefile = base64_decode($_POST['secretname']);
        $id = base64_decode($_POST['id_file']);
        $this->load->model('mglobal');

        if($tipe=='surat_berharga'){
            $dataState = $this->mglobal->get_data_by_id('t_lhkpn_harta_surat_berharga','ID',$id,false,true);
            $updateFile = str_replace($namefile.',','',$dataState->FILE_BUKTI);
            $result = $this->mglobal->update_data('t_lhkpn_harta_surat_berharga',['FILE_BUKTI'=>$updateFile],'ID', $id);
        }else if($tipe=='kas'){
            $dataState = $this->mglobal->get_data_by_id('t_lhkpn_harta_kas','ID',$id,false,true);
            $updateFile = str_replace($namefile.',','',$dataState->FILE_BUKTI);
            $result = $this->mglobal->update_data('t_lhkpn_harta_kas',['FILE_BUKTI'=>$updateFile],'ID', $id);
        }else if($tipe=='harta_lainnya'){
            $dataState = $this->mglobal->get_data_by_id('t_lhkpn_harta_lainnya','ID',$id,false,true);
            $updateFile = str_replace($namefile.',','',$dataState->FILE_BUKTI);
            $result = $this->mglobal->update_data('t_lhkpn_harta_lainnya',['FILE_BUKTI'=>$updateFile],'ID', $id);
        }else{
            $result = 0;
        }
        echo $result;
