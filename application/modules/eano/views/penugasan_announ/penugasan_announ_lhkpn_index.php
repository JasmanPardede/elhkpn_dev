 ...
 * @package Views/eano/Penugasan_announ
*/
?>
<div class="box-header with-border">
    ...
    <div class="row">         
        ...
        <div class="col-md-12">
            <form method="post" class='form-horizontal' id="ajaxFormCari" action="index.php/eano/penugasan_announ/index/lhkpn/">
                ...
                <div class="col-md-6">    
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select class="form-control" name="CARI[STAT]">
                                    <option value="-99" <?php if(@$CARI['STAT'] == '-99'){ echo 'selected';};?>>-pilih Status-</option>
                                    <option value="1" <?php if(@$CARI['STAT'] == '1'){ echo 'selected';};?>>Belum Ditugaskan</option>
                                    <option value="2" <?php if(@$CARI['STAT'] == '2'){ echo 'selected';};?>>Sudah Ditugaskan</option>
                                    <!-- <option value="3" <?php if(@$CARI['STAT'] == '3'){ echo 'selected';};?>>Sudah Diverifikasi</option> -->
                                </select>
                    ...
                    <div class="row">
                        <div class="form-group">
                            <label class="col-sm-4 control-label">Verifikator Naskah:</label>
                            <div class="col-sm-5">
                                <!-- <select name="CARI[PETUGAS]" id="CARI_PETUGAS">
                                </select> -->
                                <input type="text" name="CARI[PETUGAS]" id="CARI_PETUGAS" placeholder="Verifikator Naskah" style="width: 200px;" value="<?php echo @$CARI['PETUGAS'];?>">
                            </div>
                            <div class="col-sm-3">
...
<div class="box-body">
    ...
    <table class="table table-striped table-bordered table-hover table-heading no-border-bottom">
        ...
        <tbody>
            ...
            <tr>
                <?php if($item->STAT == '1' || $item->STAT == '') { ?>
                    <td class="showWhenEditable">
                        <span style="display: none;" class="lhkpn"><?php echo substr(md5($item->ID_LHKPN_DIJABATAN),5,8); ?></span>
                        <span style="display: none;" class="nik"><?php echo $item->NIK; ?></span>
                        <?php echo (in_array($item->ID_LHKPN_DIJABATAN, $aId) ? '<input class="chk" type="checkbox" checked="checked" value="'.$item->ID_LHKPN_DIJABATAN.'" onclick="chk(this);" style="display: none;" />' : '<input class="chk" type="checkbox" value="'.$item->ID_LHKPN_DIJABATAN.'" onclick="chk(this);" />'); ?>
                    </td>
                <?php } else { ?>
                    <td></td>
                <?php } ?>
                ...
                <td>
                    <a href="index.php/ever/penugasan_verifikasi/getInfoPn/<?php echo $item->ID_PN;?>" onClick="return getPn(this);" class="nama"><?php echo $item->NAMA; ?></a>
                </td>
                ...
                <!--td><?php if($item->TANGGAL_PENUGASAN != ''){ echo date('d/m/Y',strtotime($item->TANGGAL_PENUGASAN));} ?></td-->
                <!--td><?php if($item->DUE_DATE != ''){ echo date('d-m-Y',strtotime($item->DUE_DATE));} ?></td-->
                <td><?php if($item->STAT == '' || $item->STAT == 1){ echo 'Belum Ditugaskan';}else{ if($item->STAT == 2){ echo 'Sudah Ditugaskan.<br />verifikator: '.$item->USERNAME.' ('.date('d/m/Y',strtotime($item->TANGGAL_PENUGASAN)).')';}else{echo $item->STAT;}} ?></td>
                <!--td nowrap="">
                    <?php
                    if($item->ID_TUGAS){
                    ?>
    ...
    <br />
    <button style="float: left;margin-top: 10px;" type="button" class="btn btn-primary" onclick="f_bast();">Masukkan Ke daftar Penugasan</button>
    <br /><br/>
    <!-- <table id="con-bast" class="table table-striped table-bordered table-hover table-heading no-border-bottom" style="clear: both;margin: 10px 0;"> -->
   <table id="con-bast" class="table table-striped table-bordered table-hover table-heading no-border-bottom" style="clear: both;margin: 10px 0;">
        <thead>
        <tr>
            <th width="30">No.</th>
    ...
    <form method="post" name="ajaxFormPenugasan" id="ajaxFormPenugasan" action="index.php/eano/penugasan_announ/lhkpn/save">
        <div id="wrapperFormPenugasan" class="form-horizontal">
            <input type="hidden" name="id" value="<?php echo @$id; ?>" />
            <div class="form-group">
                <label class="col-sm-4 control-label"><?php echo @$CARI['STATUS']==2?'Ganti ':'';?> Verifikator Naskah <font color='red'>*</font>:</label>
                <div class="col-sm-8">
                    <input type="text" name="PETUGAS" id="PETUGAS" required placeholder="Verifikator Naskah" style="width: 200px;">
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    ...
                    <input type="submit" name="" class="btn btn-sm btn-default" value="Simpan">
                    <input type="reset" name="" class="btn btn-sm btn-default" value="Batal">
                </div>
            </div>
        </div>
    </form>
...
$(document).ready(function() {
        ...
        if(idChk.length != 0) {
            ...
            $.ajax({
                ...
                success : function(data, textStatus, jqXHR) {
                    ...
                    url = 'index.php/eano/penugasan_announ/index/lhkpn';
    ...
    $('#PETUGAS').select2({
        ...
        formatResult: function (state) {
            if(state.id == '0'){
                return '-- Pilih Verifikator Naskah --';
            }else{
