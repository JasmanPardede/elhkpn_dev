...
<section class="content">
  <div class="row">
    <div class="col-md-12">
    	   <form method="post" class='form-horizontal' id="ajaxFormCari" action="index.php/eano/announ/index/terverifikasi">
			...
			<div class="box-header with-border">
				...
				<div class="col-md-6">
				 <div class="row">
			                    ...
			                    <div class="row">
			                        <div class="form-group">
			                            <label class="col-sm-4 control-label">Sudah diperiksa :</label>
			                            <div class="col-sm-5">
			                                <select class="form-control" name="CARI[PERIKSA]">
			                                    <option value="">-pilih Status Periksa-</option>
			                                    <option value="1" <?php if(@$CARI['PERIKSA'] == '1'){ echo 'selected';};?>>Sudah</option>
			                                    <option value="0" <?php if(@$CARI['PERIKSA'] == '0'){ echo 'selected';};?>>Belum</option>
			                                </select>
			                                <!-- <button type="button" class="btn btn-sm btn-default">...</button> -->
			                            </div>
			                            <div class="col-sm-3">
			...
			?>
			        ...
			        <tr>
			            ...
			            <td width="120" nowrap="" align="center">
			            	...
			            	<input type="hidden" class="keyori" value="<?php echo $item->ID_LHKPN;?>">
			            	<?php
			                    if ($item->STATUS_PERBAIKAN_NASKAH == '2') {
			                    	echo '<button type="button" class="btn btn-sm btn-success btnGenPDF" href="portal/ikthisar/cetakAnnoun">Periksa Naskah</button><br/>';
			                        echo '<i class="fa fa-minus-square" style="cursor: pointer; color: red;" title="Naskah ini perlu perbaikan !"></i>';
			                    } else if($item->STATUS_PERBAIKAN_NASKAH == '1') {
			                    	echo '<i class="fa fa-check-square" style="cursor: pointer; color: blue;" title="Naskah sesuai!"></i>';
			                    } else if($item->STATUS_PERBAIKAN_NASKAH == '0') {
			                    	echo '<button type="button" class="btn btn-sm btn-success btnGenPDF" href="portal/ikthisar/cetakAnnoun">Periksa Naskah</button><br/>';
			                    }
			                ?>
...
<div style="z-index: 1060" id="stack2" class="modal hide fade" tabindex="2" data-focus-on="input:first">
...
function getPn(ele){
    ...
    $.get(url, function(html){
        OpenModalBox('Detail PN/WL', html, '', 'standart');
    });
...
$(document).ready(function() {
    ...
    $('.btnGenPDF').click(function(){
    	...
    	keyori = $(this).parents('td').children('.keyori').val();
    	url = $(this).attr('href')+'/'+keyori;
...
//            $.get('<?php echo base_url() ?>/index.php/efill/lhkpn/entry/'+key+'/view/true', function(home2){
                ...
                $('#ajaxFormPrint').css('display','none');
                html += '<div class="row"><div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 pdf-iframe" style="padding-right: 5px;border-right: 2px solid black;">'
                    + '<iframe onload="clearLoading();" src="" width="100%" height="'
                    + ($(window).height() - 140 + 'px')
                    + '" style="border:1px solid #cfcfcf;" name="iframeCetak"></iframe></div>';
//                html += '<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6 pdf-iframe2" style="padding-left: 0px;">'+home2+'</div></div>';
        console.log(keyori);
