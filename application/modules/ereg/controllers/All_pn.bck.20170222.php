...
//        var_dump($record, $this->db->last_query());exit;
        if (count($record)) {
            ...
            $this->db->select('T_PN.ID_PN, T_PN.NIK, T_PN.NAMA, JABATAN.NAMA_JABATAN,  t_pn_jabatan.DESKRIPSI_JABATAN, T_USER.ID_USER,  JAB.NAMA_JABATAN N_JAB, SUK.SUK_NAMA N_SUK, UK.UK_NAMA N_UK, INTS.INST_NAMA, ID_STATUS_AKHIR_JABAT', FALSE);
            $this->db->from('T_PN');
...
//        $this->load->library('lexcel');
        ...
        if ($read_excel_ok && $response_data->load_data_success) {
            ...
            if ($total_record > 1) {
                ...
                foreach ($response_data->records as $data):
                    $cOnline = trim($data["row_contents"][7]);
                    ...
                    $data_xls = array(
                        ...
                        'IS_ONLINE' => $cXonline,
                        'EMAIL' => $data["row_contents"][8],
                        'NO_HP' => trim(str_replace("'", "", trim($data["row_contents"][9]))),
                        'NIK_BARU' => trim(str_replace("'", "", $data["row_contents"][10])),
                        'TEMPAT_LAHIR' => addslashes($data["row_contents"][12]),
                        'TGL_LAHIR' => string_to_date($data["row_contents"][13], 'Y-m-d'),
                        'JNS_KEL' => $data["row_contents"][11],
                        'IS_PROCESSED' => '0',
                        'CREATED_TIME' => time(),
                        'CREATED_BY' => $this->session->userdata('USR'),
                        'CREATED_IP' => $_SERVER["REMOTE_ADDR"]
    ...
    public function DownUpExcels() {
        ...
        $data = array(
            ...
            'list_ver_pnwl' => $this->Mt_pn_ver_pnwl->get_count_ver_pnwl_jabatan($ins),
            'list_ver_pnwl_tambah' => $this->Mt_pn_ver_pnwl->get_count_ver_pnwl_tambahan($ins),
...
//
    ...
    function saveUploadExl() {
        ...
        try {
            $save_excel_berhasil = $this->UpExcelload($xexcel_filename);
        } catch (Exception $ex) {
            $save_excel_berhasil = FALSE;
        }

        if ($save_excel_berhasil) {
...
//        var_dump($xlTableRowCount, );exit;
    ...
    function __write_list_data_to_excel($ins, $uk_nama, $n_instansi) {

        $total_record = $this->mpn->__count_get_all($ins, $uk_nama);
        if ($total_record > 0) {
            $page = 1;
            $current_execution = 1;
            ...
            while ($current_execution <= $exec_times) {
                $offset = $this->__calculate_excel_offset($current_execution, $this->limit_write_records);
                $result_list_data = $this->mpn->get_all_limited($ins, $uk_nama, $this->limit_write_records, $offset);
                foreach ($result_list_data as $dataRow) {
    ...
    private function __uniqueRandomNumbersWithinRange($min, $max, $quantity) {
...
//        $list_data = array();
//        $this->__execution_time = 'min';
        $this->load->model('Minstansi', '', TRUE);
        $instansi = $this->Minstansi->get_by_id($ins)->row();
        $n_instansi = $instansi->INST_NAMA;
        $kode = $instansi->INST_SATKERKD;
...
//        $unique_id = $this->__uniqueRandomNumbersWithinRange(100, 200, (int) date('H'));
        $arr_workSheets = [];
        $workSheetPnwl = $this->__write_list_data_to_excel($ins, $uk_nama, $n_instansi);
        if ($workSheetPnwl !== FALSE) {
            $arr_workSheets[] = $workSheetPnwl;
        }
        $workSheetInstansi = $this->__write_list_data_instansi_to_excel($ins);
        if ($workSheetInstansi !== FALSE) {
            $arr_workSheets[] = $workSheetInstansi;
        }
        $workSheetUnitKerja = $this->__write_list_data_unit_kerja_to_excel($ins);
        if ($workSheetUnitKerja !== FALSE) {
            $arr_workSheets[] = $workSheetUnitKerja;
        }
        $workSheetSubUnitKerja = $this->__write_list_data_sub_unit_kerja_to_excel($ins);
        if ($workSheetSubUnitKerja !== FALSE) {
            $arr_workSheets[] = $workSheetSubUnitKerja;
        }
        $workSheetJabatan = $this->__write_list_data_jabatan_to_excel($ins);
        if ($workSheetJabatan !== FALSE) {
            $arr_workSheets[] = $workSheetJabatan;
        }


        $workSheets = implode(" ", $arr_workSheets);
        $this->__save_file_excel($workSheets, $kode);
//        echo $file_name_excel;


        /**
         * ini bisa jadi referensi header
         */
