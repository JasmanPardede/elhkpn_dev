...
//        display($this->session->userdata('IS_UK'));
        ...
        //IBO Update
        $this->load->model('Mmpnwn');
        $min_tahun = $this->Mmpnwn->get_min_tahun_t_pn();
        $default_cari_tahun = (date('m') <= 10) ? date('Y') - 1 : date('Y');
        $default_instansi = $this->instansi;
        if (isset($this->CARI) && array_key_exists('TAHUN_WL', $this->CARI) && $this->CARI['TAHUN_WL']) {
            $default_cari_tahun = $this->CARI['TAHUN_WL'];
            $default_instansi = $this->CARI['INSTANSI'];
        }
        //End IBO Update
        $data = array(
            'linkCetak' => 'index.php/ereg/all_pn/index/0',
            ...
            'havelhkpn' => $havelhkpn,
            'min_tahun' => $min_tahun,
            'default_cari_tahun' => $default_cari_tahun,
        ...
        if (@$this->iscetak) {
            ...
            $data['js_page'] = $this->load->view(strtolower(__CLASS__) . '/js/' . strtolower(__FUNCTION__) . '_js', $data, TRUE);
    ...
    public function savepnwl($calon) {
        $this->db->trans_begin();
        ...
        // $IS_APLIKASI = $this->input->post('is_aplikasi', TRUE);
        $TAHUN_WL = $this->input->post('TAHUN_WL', TRUE);
        ...
        $detected_role = ID_ROLE_PN;
        $detected_destination_inst = $this->input->post('LEMBAGA', TRUE);
...
//        if ($insert) {
        if ($USERNAME != '' || $USERNAME != null){
            $cek_nik = $this->muser->check_user_if_exist($USERNAME);
        }
...
//        }
        ...
        if ($res >= 1) {
            if($calon == 2) {
                ng::logActivity('Tambah PN/WL, NIK = ' . $this->input->post('NIK', TRUE));
            } else {
                ng::logActivity('Tambah Calon PN/WL, NIK = ' . $this->input->post('NIK', TRUE));
            }
        }
        return $res;
        exit;
...
//        $hapus = $this->mglobal->delete_data('ID_PN', $id_pn, 'T_PN_JABATAN');
     ...
     */
    public function detailpn($id, $idjb = NULL, $tahun_wl) { 
...
//                return false;
        ...
        $riwayat_jabatan = $this->mglobal->get_riwayat_jabatan($idjb);

        // ambil data lhkpn berdasarkan tahun wl
        $this->db->select('ID_LHKPN');
            ...
            $this->db->limit(1);
            $lhkpn = $this->db->get()->row(); 
        
        if($lhkpn){
            $joinKeluarga = [
                ['table' => 'T_LHKPN', 'on' => 'T_LHKPN_KELUARGA.ID_LHKPN = T_LHKPN.ID_LHKPN', 'JOIN' => 'LEFT']
             ];
             $selectKeluarga = 'T_LHKPN_KELUARGA.ID_KELUARGA, T_LHKPN_KELUARGA.ID_LHKPN, T_LHKPN_KELUARGA.NAMA, T_LHKPN_KELUARGA.HUBUNGAN, T_LHKPN_KELUARGA.NIK, FLOOR(DATEDIFF(NOW(),(T_LHKPN_KELUARGA.TANGGAL_LAHIR))/365) AS UMUR, FLOOR(DATEDIFF(T_LHKPN.TGL_KIRIM_FINAL,(T_LHKPN_KELUARGA.TANGGAL_LAHIR))/365) AS UMUR2, T_LHKPN.STATUS, T_LHKPN.ENTRY_VIA, FLAG_SK';
             $keluarga_pn = $this->mglobal->get_data_all('T_LHKPN_KELUARGA', $joinKeluarga, ['T_LHKPN_KELUARGA.ID_LHKPN' => $lhkpn->ID_LHKPN, 'T_LHKPN_KELUARGA.IS_ACTIVE' => '1'], $selectKeluarga, NULL, ['T_LHKPN_KELUARGA.ID_KELUARGA','ASC']);
        }
        ...
        $data = array(
            ...
            'getKeluarga' => ($lhkpn)?$keluarga_pn:null,
            'arr2' => array(),
            'have_lhkpn' => ($lhkpn)?true:false
        );
        ...
        $namaform = 'detail';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }
...
//        display($this->session->userdata('IS_UK'));
        ...
        // prepare paging
        $this->base_url = 'index.php/ereg/all_pn/nonaktif/';
        $this->uri_segment = 4;
        $this->offset = $this->uri->segment($this->uri_segment);
        //IBO Update
        $this->load->model('Mmpnwn');
        ...
        $data['url_get_lembaga'] = base_url('index.php/share/reff/getLembaga');

        $data['js_page'] = $this->load->view('all_pn/js/all_pn_non_aktif/all_pn_non_aktif_js', $data_js, TRUE);
//echo strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_' . strtolower(__FUNCTION__);exit;
        if (@$this->iscetak) {
             ...
             */
            $this->load->view('all_pn/all_pn_nonaktif', $data);
        }
    }
...
//        $hapus = $this->mglobal->delete_data('ID_PN', $id_pn, 'T_PN_JABATAN');
    ...
    public function kirimaktivasi($id_user) {
        ...
        $data_form = $data;
        $namaform = 'kirim_aktivasi';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }
    /* Ibo Add 15 Agustus 2017   */
    public function TerimaFormulir($ID_PN) {
        $data = array(
            'form' => 'batalterimaformulir',
            'item' => $this->mglobal->get_data_all('T_PN', NULL, NULL, '*', "ID_PN = '$ID_PN'")[0],
        );
        $data_form = $data;
        $namaform = 'terimaformulir';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }
    public function BatalTerimaFormulir($ID_PN) {
        $data = array(
            'form' => 'batalterimaformulir',
            'item' => $this->mglobal->get_data_all('T_PN', NULL, NULL, '*', "ID_PN = '$ID_PN'")[0],
        );
        $data_form = $data;
        $namaform = 'batalterimaformulir';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }
    public function doTerima(){
        $ID_PN = $this->input->post('ID_PN');
        $status = array('IS_FORMULIR_EFILLING' => 1,'TGL_TERIMA_FORMULIR' => date('Y-m-d'));
        $this->mglobal->update_data_('T_PN', $status, 'ID_PN', $ID_PN);
        echo 1;
    }
    public function doTidakTerima(){
        $ID_PN = $this->input->post('ID_PN');
        $status = array('IS_FORMULIR_EFILLING' => 0,'TGL_TERIMA_FORMULIR' => date('Y-m-d'));
        $this->mglobal->update_data_('T_PN', $status, 'ID_PN', $ID_PN);
        echo 1;
    }
    /* * */
     ...
     */
    public function __do_reset_password() {
        $id_user = $this->input->post('ID_USER');
        $this->makses->check_is_write();
        $this->load->model('muser');
     ...
     * @return html Penyelenggara Negara List
     */
    public function calonpn_showdata($page_mode = FALSE){
        list($currentpage, $rowperpage, $keyword, $state_active, $sort) = $this->get_param_load_paging_default();

        if (!$page_mode)
            $page_mode = $this->input->get('page_mode');

        $response = $this->mpn->get_daftar_Calonpn($this->instansi, $currentpage, $keyword, $rowperpage, $page_mode);
        $dtable_output = array(
            "sEcho" => intval($this->input->get("sEcho")),
            "iTotalRecords" => intval($response->total_rows),
            "iTotalDisplayRecords" => intval($response->total_rows),
...
//        var_dump($record, $this->db->last_query());exit;
        if (count($record)) {
            ...
            $this->db->select('T_PN.ID_PN, T_PN.NIK, T_PN.NAMA, JAB.NAMA_JABATAN,  t_pn_jabatan.DESKRIPSI_JABATAN, T_USER.ID_USER,  JAB.NAMA_JABATAN N_JAB, SUK.SUK_NAMA N_SUK, UK.UK_NAMA N_UK, INTS.INST_NAMA, INTS.INST_SATKERKD, ID_STATUS_AKHIR_JABAT,t_pn_jabatan.LEMBAGA,t_pn_jabatan.IS_WL,t_pn_jabatan.tahun_wl, t_pn_jabatan.ID', FALSE);
            $this->db->from('T_PN');
            ...
            $year_n = date("Y");
            $join = [
                ['table' => 'M_JABATAN', 'on' => 'M_JABATAN.ID_JABATAN = T_PN_JABATAN.ID_JABATAN', 'join' => 'LEFT'],
                ['table' => 'M_INST_SATKER', 'on' => 'M_INST_SATKER.INST_SATKERKD = M_JABATAN.INST_SATKERKD', 'join' => 'LEFT'],
            ];
            $where_jab =  array(
                "T_PN_JABATAN.ID_PN" => $record[0]->ID_PN,
                "T_PN_JABATAN.IS_CALON" => '0',
                "T_PN_JABATAN.IS_ACTIVE" => '1',
                "T_PN_JABATAN.IS_DELETED" => '0',
                "T_PN_JABATAN.IS_CURRENT" => '1',
                "M_JABATAN.INST_SATKERKD" => $this->session->userdata('INST_SATKERKD'),
            );
            ...
            $where_jab2 =  array(
                ...
                "T_PN_JABATAN.IS_DELETED" => '0',
                "T_PN_JABATAN.IS_CURRENT" => '1',
                "T_PN_JABATAN.IS_WL" => '1'
            );

            $where_cek_tahun_wl = '(TAHUN_WL IS NOT NULL AND TAHUN_WL <> \'\' AND TAHUN_WL = \''.$year_n.'\')';
            $data['cek_rangkap'] = $this->mglobal->get_data_all('T_PN_JABATAN', $join, $where_jab, '*', $where_cek_tahun_wl, NULL, NULL, '1')[0];
            // $data['cek_rangkap'] = ($cek_rangkap) ? TRUE : FALSE;

            $data['cek_rangkap2'] = $this->mglobal->get_data_all('T_PN_JABATAN', $join, $where_jab2, '*', $where_cek_tahun_wl, NULL, NULL, '1')[0];
            $data['cek_rangkap2'] = ($data['cek_rangkap2']) ? true : false;
     ...
     */
    public function UpExcelload($upXlsFile = NULL, $instansi_found, $instansi_id = FALSE, $posted_uk_id = FALSE) {
...
//        $this->load->library('lexcel');
        ...
        /**
         * diupload untuk seluruh instansi
         */
        $is_instansi = '1';
        $bool_is_instansi = TRUE;

        if ($posted_uk_id && !is_null($posted_uk_id) && !empty($posted_uk_id)) {
            $detail_uk = $this->Munitkerja->get_by_id($posted_uk_id, FALSE);
            if ($detail_uk) {
                $nama_uk_detail = $detail_uk->UK_NAMA;
            }
            /**
             * diupload hanya untuk unti kerja
             */
            $is_instansi = '0';
            $bool_is_instansi = FALSE;
        }
...
//                echo count($response_data->records)."<br />";
//
//                var_dump($response_data->records[12]);
//                echo "<br />";

                foreach ($response_data->records as $key_index => $data):
...
//var_dump($data_xls);echo "<br /><br /><br />";
                    $data_xls = array(
                        ...
                        'NAMA' => addslashes(html_entity_decode(strtoupper($data["row_contents"][3]))),
                        'NAMA_UNIT_KERJA' => addslashes(html_entity_decode(strtoupper($data["row_contents"][4]))),
                        'NAMA_SUB_UNIT_KERJA' => addslashes(html_entity_decode(strtoupper($data["row_contents"][5]))),
                        'ID_LEMBAGA' => $instansi_id,
                        'NAMA_LEMBAGA' => addslashes(html_entity_decode(strtoupper($instansi->INST_NAMA))),
                        'NAMA_JABATAN' => strtoupper(html_entity_decode($data["row_contents"][6])),
                        'EMAIL' => $data["row_contents"][7],
                        'NO_HP' => trim(str_replace("'", "", trim($data["row_contents"][8]))),
                        'JNS_KEL' => $data["row_contents"][9],
                        'TEMPAT_LAHIR' => addslashes(html_entity_decode($data["row_contents"][10])),
                        'TGL_LAHIR' => string_to_date($data["row_contents"][11], 'Y-m-d'),
                        'IS_PROCESSED' => '0',
                        'CREATED_TIME' => time(),
                        'CREATED_BY' => $this->session->userdata('USR'),
                        'CREATED_IP' => $_SERVER["REMOTE_ADDR"],
                        'upload_as_instansi' => $is_instansi
                    );
...
//                    get_uk_id_by_name_and_lembaga_id
                    ...
                    if (($id_unit_kerja_found && $bool_is_instansi) || ($id_unit_kerja_found && !$bool_is_instansi && $id_unit_kerja_found == $posted_uk_id)) {
                        $data_xls['uk_id_detected'] = $id_unit_kerja_found;
                    } elseif ($id_unit_kerja_found && !$bool_is_instansi && $id_unit_kerja_found != $posted_uk_id) {
                        $arr_response = [
                            "success" => FALSE,
                            "err_code" => "",
                            "msg" => "Ditemukan data NIK yang tidak seharusnya terdapat pada Unit Kerja " . $nama_uk_detail . "."
                        ];
                        $this->log_message("info", "all_pn.php Ditemukan data NIK yang tidak seharusnya terdapat pada Unit Kerja " . $nama_uk_detail . ".");
                        $uk_data_valid = FALSE;
                        break;
                    } else {
                        $arr_response = [
...
//                    echo "ada adi".$key_index."<br />";
                    ...
                    if ($required_data_ok && $uk_data_valid) {
                        ...
                        $arr_response = [
                            "success" => FALSE,
                            "err_code" => "",
                            "msg" => "Data Excel yang diupload belum lengkap, cek kembali isian data pada excel Anda."
                        ];
                        break;
                    }

//                    echo "ada adu".$key_index."<br />";
...
//
    ...
    function saveUploadExl() {
        ...
        $posted_uk_id = $this->input->post('CARI[UNIT_KERJA]');
        ...
        try {
            $save_excel_berhasil = $this->UpExcelload($xexcel_filename, $data['instansi_found'], $instansi_id, $posted_uk_id);
        } catch (Exception $ex) {
//            $save_excel_berhasil = FALSE;
    ...
    function load_data_daftar_pn_individu($page_mode = FALSE) {
...
// echo $this->db->last_query();exit;
    ...
    function daftar_pn_individu($cetak = FALSE, $show_form_entry = FALSE, $ok = FALSE) {
        ...
        $data = array(
            'breadcrumb' => call_user_func('ng::genBreadcrumb', array(
                'Dashboard' => 'index.php/welcome/dashboard',
                'E-reg' => 'index.php/dashboard/efilling',
                'PN' => 'index.php/all_pn/daftar_pn_individu',
            )),
            'iscln' => 0,
        );
...
//        $data['daftar_pn'] = $this->mpn->get_daftar_pn_individual_two($this->instansi, $offset, $cari);
        ...
        $data['show_form_entry'] = $show_form_entry;

        if ($show_form_entry && !$ok) {
            $url_to = base_url() . 'index.php#index.php/ereg/all_pn/daftar_pn_individu/0/daftarindividu/1';
    ...
    function daftar_pn_individu_ver($offset = 0, $cetak = false) {
        ...
        $data['daftar_pn'] = $this->mpn->get_daftar_pn_individual($this->instansi, $cari);
        $data['daftar_calon_pn'] = $this->mpn->get_daftar_jml_calonpn($this->instansi, $cari);
...
//        $this->load->model('munitkerja', '', TRUE);
    ...
    public function editpn_daftar($iscln = NULL, $status = NULL, $id = NULL) {
        ...
        if($iscln==1){
            $data['daftar_pn'] = $this->mglobal->daftar_pn_individual_by_id_isCalon_1($id);
        }
        else{
...
//        echo strtolower(__CLASS__) . '/js/daftar_pn_individu_form_js';exit;
        $data_js = $data;
        $data['js_page'][] = $this->load->view('all_pn/js/daftar_pn_individu_form_js', $data, TRUE);
        $data['js_page'][] = $this->load->view('all_pn/js/submit_daftar_pn_individu_form_js', $data, TRUE);

        $this->load->view('all_pn/form_edit_daftar_pn', $data);
    }
...
//            'NIK' => $NIK,
    ...
    public function delete_daftar_pn_individual($id_pn, $calon = NULL) {
        $data = array(
        ...
        $data_form = $data;
        $namaform = 'delete_daftar_pn_individual';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }
    ...
    public function delete_vi($id_pn, $id_j) {
        ...
        $data_form = $data;
        $namaform = 'delete_verifikasi_data_individu';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }

    function do_delete_daftar_pn_individual($id_pn, $calon = NULL) {
        $this->load->model('mglobal', '', TRUE);
        $data_pn = $this->mglobal->get_by_id('T_PN', 'ID_PN', $id_pn);
...
//        $this->mglobal->update_data_('T_PN_JABATAN', $data, 'ID_PN', $id);
        ...
        if ($hapus) {
            echo 1;
            if($calon == 1) {
                ng::logActivity('Pembatalan Penambahan Calon PN/WL, NIK = ' . $data_pn->NIK);
            } else {
                ng::logActivity('Pembatalan Penambahan PN/WL, NIK = ' . $data_pn->NIK);
            }
        } else {
            echo 0;
        }
...
//        $simpan = $this->mglobal->update_data_('T_PN', $arr_pn, 'ID_PN', $id);
        ...
        $double_jabatan = FALSE;
        // $tahun_min1 = date("Y") - 1;
        $tahun_n = date("Y");
        $where_cek = ['ID_PN' => $id, 'ID_JABATAN' => $id_jabatan, 'IS_CALON' => '0', 'IS_ACTIVE' => '1', 'IS_DELETED' => '0', 'IS_CURRENT' => '1'];
        $where_cek_tahun_wl = '(TAHUN_WL IS NOT NULL AND TAHUN_WL <> \'\' AND TAHUN_WL = '.$tahun_n.')';
        $cek_id_jabatan = $this->mglobal->get_data_all('T_PN_JABATAN', NULL, $where_cek, '*', $where_cek_tahun_wl, NULL, '0', '1');
        if ($cek_id_jabatan) {
            $double_jabatan = TRUE;
            $save = FALSE;
        } else {
    ...
    function __send_mail_approval_verification($id, $idj, $content) {
        ...
        if ($password) {
            ...
            if ($data_pn && $user_uk) {
                ...
                $hasil_kirim_info_akun = $this->muser->kirim_info_akun($user_uk->EMAIL, $user_uk->USERNAME, $password, $user_uk->NAMA, $user_uk->INST_NAMA, $user_uk->ID_USER, $user_uk->PASSWORD);
            }
            exit;
        } else {
            echo "0";
...
//        $url = base_url() . "index.php/ereg/all_pn_mail/" . $actionMethod . "/" . $id . "/" . $idj . "/" . $content;
        $url = "https://elhkpn.kpk.go.id/index.php/ereg/all_pn_mail/" . $actionMethod . "/" . $id . "/" . $idj . "/" . $content;
...
// close cURL resource, and free up system resources
    ...
    // Verifikasi Individu
    function app_vi_add($id, $idj, $calon = NULL) {
        $this->db->trans_begin();
        $user_uk = FALSE;
        ...
        if ($data_pn) {
            ...
            $salt = $this->muser->get_by_id($id)->row()->SALT; 
            if($salt == null){
                $salt = random_word(8);
            }
        ...
        if ($data_pn && $user_uk) {
            ...
            $email = $data_pn->EMAIL;
            $is_aplikasi = $data_pn->IS_APLIKASI;
...
//            if ($email == NULL || $email == '') {
            if ($is_aplikasi == NULL || $is_aplikasi == '') {
                $dt_pn_jab = array(
                    'IS_CURRENT' => 1,
                    'ID_STATUS_AKHIR_JABAT' => 5,
                    'IS_ACTIVE' => 1,
...
//            if ($is_aplikasi != NULL || $is_aplikasi != '') {
//                $this->__send_mail_approval_verification($id, $idj, $password);
//            }
            ...
            if ($res == 1) {
                ...
                if($calon == 1) {
                    ng::logActivity('Verifikasi Penambahan Calon PN/WL, NIK = ' . $data_pn->NIK);
                } else {
                    ng::logActivity('Verifikasi Penambahan PN/WL, NIK = ' . $data_pn->NIK);
                }
...
//        if ($data_pn && $user_uk && $data_pn_jab) {
        if ($data_pn && $data_pn_jab) {
            ...
            $this->db->where('ID_PN', $id);
            $this->db->where('tahun_wl', $data_pn_jab->tahun_wl);
            $this->db->where('tahun_wl is not', null);
            $this->db->update('t_pn_jabatan', $dt_pn_);
            $this->mglobal->update_data_('t_pn_jabatan', $dt_pn_jab, 'ID', $idj);
            /*
             * jika mengonlinekan tahun 2017 maka 2018 ikut online, tidak sebaliknya
             */
            $this->db->where('LEMBAGA', $data_pn_jab->LEMBAGA);
            $this->db->where('ID_PN', $id);
            $this->db->where('is_active', 1);
            ...
            $this->db->where('tahun_wl is not', null);
            $this->db->update('t_pn_jabatan', $dt_pn_jab);
            /*
             * end
             */
            ...
            if ($asd == "") {
                $data_update = array(
                    ...
                    'NAMA' => $data_pn->NAMA,
                    'INST_SATKERKD' => $data_pn_jab->LEMBAGA,
            ...
            if ($res == 1) {
                ...
                // ng::logActivity('Verifikasi Perubahan PN/WL, NIK = ' . $data_pn->NIK);
                ng::logActivity('Verifikasi Mengonlinekan PN, ID PN = ' . $id);
    ...
    function app_vi_nonact($id, $idj) {
        ...
        if ($res >= 1) {
            ...
            ng::logActivity('Verifikasi Me Wajib Laporkan, ID PN = ' . $id);
    ...
    function app_vi_nonact_new($id, $idj) {
        ...
        if ($res >= 1) {
            ...
            ng::logActivity('Verifikasi Me Non Wajib Laporkan, ID PN = ' . $id);
...
//        return $res;
        if ($res == 1)
            ...
            ng::logActivity('Pembatalan Mengonlinekan PN, ID PN = ' . $id);
...
//            'ID_STATUS_AKHIR_JABAT' => 0,
            'IS_WL' => 1,
            'ALASAN_NON_WL' => NULL // /*ab*/ menyisipkan query ketika cancel verivikasi (fild ybs menjadi null nilainya)
        );
//        $this->mglobal->update_data_('t_pn', $dt_pn, 'ID_PN', $id);
        $this->mglobal->update_data_('t_pn_jabatan', $dt_pn_jab, 'ID', $idj);
        ...
        $res = intval($this->db->trans_status());
        ng::logActivity('Pembatalan Me Non Wajib Laporkan, ID PN = ' . $id);
        return $res;
    }
...
//        $this->mglobal->update_data_('t_pn', $dt_pn, 'ID_PN', $id);
        ...
        $res = intval($this->db->trans_status());
        ng::logActivity('Pembatalan Me Wajib Laporkan, ID PN = ' . $id);
        return $res;
    }
    ...
    public function wl_aktif($offset = 0, $cetak = false) {
        ...
        $this->base_url = 'index.php/ereg/all_pn/wl_aktif/';
        $this->uri_segment = 4;
        $this->offset = $this->uri->segment($this->uri_segment);
        ...
        $data = array(
            ...
            'breadcrumb' => call_user_func('ng::genBreadcrumb', array(
                'Dashboard' => 'index.php/welcome/dashboard',
                'E-reg' => 'index.php/dashboard/efilling',
                'Wajib Lapor' => 'index.php/all_pn/wl_aktif',
            )),
            'pagination' => call_user_func('ng::genPagination'),
            'instansis' => $this->mglobal->get_data_all('M_INST_SATKER'),
            'status_akhir' => $this->mglobal->get_data_all('T_STATUS_AKHIR_JABAT', NULL, NULL, '*', NULL, ['ORDER', 'asc']),
            ...
            'default_instansi' => $default_instansi,
            'iscln' => 0,
            'item' => $this->mglobal->get_data_all('T_PN',[['table' => 'T_PN_JABATAN', 'on' => 'T_PN_JABATAN.ID_PN = ' . 'T_PN.ID_PN']],NULL,"COUNT(DISTINCT(T_PN.ID_PN)) AS T_TAHUN_WL","T_PN.IS_ACTIVE = '1' AND T_PN_JABATAN.IS_ACTIVE = '1' AND IS_WL = '1' AND IS_DELETED = '0' AND IS_CURRENT = '1' AND ID_STATUS_AKHIR_JABAT IN (0,5) AND tahun_wl = '$thndpn'",NULL,0,NULL,NULL)[0]->T_TAHUN_WL
        );
        ...
        if (@$this->iscetak) {
            ...
            $data['js_page'] = $this->load->view('all_pn/js/wl_aktif/wl_aktif_js', $data, TRUE);
            $this->load->view('all_pn/all_pn_wl_aktif', $data);
        }
    }
    ...
    public function wl_nonaktif($offset = 0, $cetak = false) {
        ...
        $this->base_url = 'index.php/ereg/all_pn/wl_nonaktif/';
        $this->uri_segment = 4;
        $this->offset = $this->uri->segment($this->uri_segment);
        /* New By Rian */
        $this->load->model('Mmpnwn');
        ...
        $data = array(
            ...
            'breadcrumb' => call_user_func('ng::genBreadcrumb', array(
                'Dashboard' => 'index.php/welcome/dashboard',
                'E-reg' => 'index.php/dashboard/efilling',
                'Wajib Lapor' => 'index.php/all_pn/wl_nonaktif',
            )),
            'pagination' => call_user_func('ng::genPagination'),
            'instansis' => $this->mglobal->get_data_all('M_INST_SATKER'),
            'status_akhir' => $this->mglobal->get_data_all('T_STATUS_AKHIR_JABAT', NULL, NULL, '*', NULL, ['ORDER', 'asc']),
        ...
        if (@$this->iscetak) {
            ...
            $data['js_page'] = $this->load->view('all_pn/js/wl_nonaktif/wl_nonaktif_js', $data, TRUE);

            $this->load->view('all_pn/all_pn_wl_nonaktif', $data);
        }
    }
    ...
    public function editpn_daftar_wl($iscln = NULL, $status = NULL, $id = NULL, $idj = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mjabatan', '', TRUE);
        ...
        $data['daftar_pn'] = $this->mglobal->daftar_pn_individual_by_id($id,$idj);
        //var_dump($data['daftar_pn']);exit;
        if ($data['daftar_pn']) {
            $data['daftar_pn']->EMAIL = $data['daftar_pn']->tpn_email;
        }
    ...
    public function editpn_daftar_wl_nonaktif($iscln = NULL, $status = NULL, $id = NULL, $idj = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mjabatan', '', TRUE);
    ...
    public function editpn_daftar_wl_nonaktifx($iscln = NULL, $status = NULL, $id = NULL, $idj = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mjabatan', '', TRUE);
    ...
    function save_edit_daftar_wl($id,$idj) {
        $this->load->model('mglobal', '', TRUE);
        $NIK = $this->input->post('NIK');
        $NIK_BARU = $this->input->post('NIK_BARU');
        //save_edit_daftar_wl
        ...
        if ($NIK_BARU != '' || $NIK_BARU != null){
            $cek_nik = $this->mglobal->cek_nik($NIK_BARU);
        }
        ...
        if ($cek_nik == FALSE){
            ...
            if($NIK_BARU){  
                 $sql_get_lhkpn = "SELECT * FROM t_lhkpn WHERE ID_PN = '.$id.'";
                 $act_get_lhkpn = $this->db->query($sql_get_lhkpn)->result();

                 foreach($act_get_lhkpn as $state_lhkpn){
                    $this->db->where('ID_LHKPN', $state_lhkpn->ID_LHKPN);
                    $update_result = $this->db->update('t_lhkpn_data_pribadi', array(
                        'NIK' => $NIK_BARU,
                        'NIK_LAMA' => $NIK,
                    ));
                }
        ...
        //  ---------- validasi ID JABATAN double ----------
        $where_cek_id_jabatan = ['ID_PN' => $id, 'ID_JABATAN' => $id_jabatan, 'ID_STATUS_AKHIR_JABAT' => '1', 'IS_CALON' => '0', 'IS_ACTIVE' => '1', 'IS_DELETED' => '0', 'IS_CURRENT' => '1'];
        $cek_rangkap = $this->mglobal->get_data_all('T_PN_JABATAN', NULL, $where_cek_id_jabatan, 'ID');
        if ($cek_rangkap) {
            $obj = [];
            $non_active = array(
                'IS_ACTIVE' => '0',
            ...
            foreach ($cek_rangkap as $as) {
                $obj[] = $as->ID;
            }
            $this->db->where('ID IN ('.implode(",", $obj).')', NULL, FALSE);
            $this->db->update('T_PN_JABATAN', $non_active);
        }
        //  ---------- validasi ID JABATAN double ----------
...
//            'TAHUN_WL' => $TAHUN_WL,
        ...
        $this->mglobal->update_data_('T_PN_JABATAN', $jabatan, 'ID', $idj);

        if ($simpan) {
            ng::logActivity('Edit PN/WL, NIK = ' . $NIK);
            echo '1';
        } else {
            echo '0';
        }
    ...
    function save_edit_daftar_wl_non($id,$idj) {
        $this->load->model('mglobal', '', TRUE);
        ...
        $data = $this->mglobal->get_by_id('m_jabatan', 'ID_JABATAN', $id_jabatan);
        $data_jab = $this->mglobal->get_by_id('t_pn_jabatan', 'ID', $idj);
        $f_jabatan = $data->NAMA_JABATAN;
        $f_eselon = $data->KODE_ESELON;
...
//            'UPDATED_IP' => $_SERVER["REMOTE_ADDR"],
            'TMT' => $data_jab->TMT,
            'ID_STATUS_AKHIR_JABAT' => $data_jab->ID_STATUS_AKHIR_JABAT,
            'IS_ACTIVE' => $data_jab->IS_ACTIVE,
            'IS_CURRENT' => $data_jab->IS_ACTIVE,
//            'TAHUN_WL' => $TAHUN_WL,
            'TAHUN_WL' => date('Y'),
            'IS_WL' => 90,
        );
        $jabatan2 = array(
            ...
            'IS_ACTIVE' => $data_jab->IS_ACTIVE,
            'IS_CURRENT' => $data_jab->IS_ACTIVE,
            'TAHUN_WL' => $data_jab->tahun_wl,
            'IS_WL' => 90,
        );
        $status_wl = '0';
        $tahun_min_2 = date('Y')-2;
        $where_cek = ['ID_PN' => $id, 'tahun_wl' => date('Y'), 'IS_CALON' => '0', 'IS_ACTIVE' => '1', 'IS_DELETED' => '0', 'IS_CURRENT' => '1'];
        $cek_data = $this->mglobal->get_data_all('T_PN_JABATAN', NULL, $where_cek, '*', NULL, NULL, '0', '1');
        ...
        // if ($data_jab->tahun_wl <= $tahun_min_2) {
        if ($data_jab->tahun_wl != date('Y')) {
            if (!empty($cek_data[0]->is_wl)){
                $status_wl = $cek_data[0]->is_wl;
            } else {
                $this->mglobal->insert('T_PN_JABATAN', $jabatan);
            }
        } else {
            $this->mglobal->update_data_('T_PN_JABATAN', $jabatan2, 'ID', $idj);
        }
...
//        $this->mglobal->update_data_('T_PN_JABATAN', $jabatan, 'ID_PN', $id);
        ...
        if ($res >= 1) {
            ...
            ng::logActivity('Me Wajib Laporkan, ID PN = ' . $id);
        }
        if ($simpan) {
            if ($status_wl == 0) {
                echo '1';
    ...
    public function deletepn_wl($id, $idjpn = NULL) {
        ...
        $data = array(
            ...
            'idjpn' => $idjpn,
            'item' => $this->mpn->get_by_id($id)->row(),
            'tahun_wl' => $this->mglobal->get_data_all('t_pn_jabatan',null,['ID' => $idjpn],'tahun_wl'),
        );
        $data_form = $data;
        $namaform = 'delete_wl';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
        echo ($data['html_form_view']);
        exit;
    }

    public function generate_wl() {
        $this->load->model('mglobal', '', TRUE);
        $thndpn = date('Y')+1;
        $curryear = date('Y');
        $data = array(
            'form' => 'generate_wl',
            'item' => $this->mglobal->get_data_all('T_PN',[['table' => 'T_PN_JABATAN', 'on' => 'T_PN_JABATAN.ID_PN = ' . 'T_PN.ID_PN']],NULL,"COUNT(DISTINCT(T_PN.ID_PN)) AS T_TAHUN_WL","T_PN.IS_ACTIVE = '1' AND T_PN_JABATAN.IS_ACTIVE = '1' AND IS_WL = '1' AND IS_DELETED = '0' AND IS_CURRENT = '1' AND ID_STATUS_AKHIR_JABAT IN (0,5) AND tahun_wl = '$thndpn'",NULL,0,NULL,NULL)[0]->T_TAHUN_WL,
            'wlAktif' => $this->mglobal->get_data_all('T_PN',[['table' => 'T_PN_JABATAN', 'on' => 'T_PN_JABATAN.ID_PN = ' . 'T_PN.ID_PN']],NULL,"COUNT(DISTINCT(T_PN.ID_PN)) AS T_CURR_WL","T_PN.IS_ACTIVE = '1' AND T_PN_JABATAN.IS_ACTIVE = '1' AND IS_WL = '1' AND IS_DELETED = '0' AND IS_CURRENT = '1' AND ID_STATUS_AKHIR_JABAT IN (0,5) AND tahun_wl = '$curryear'",NULL,0,NULL,NULL)[0]->T_CURR_WL,
        );
        $data_form = $data;
        $namaform = 'generate_wl';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
        echo ($data['html_form_view']);
        exit;
    ...
    public function generatewl($idjpn){
        $thndpn = date('Y')+1;
        $now = date('Y-m-d');
        $curryear = date('Y');

        $sql = "insert into t_pn_jabatan (ID_PN, ID_JABATAN, DESKRIPSI_JABATAN, ESELON, NAMA_LEMBAGA, LEMBAGA, UNIT_KERJA, SUB_UNIT_KERJA, ALAMAT_KANTOR, EMAIL_KANTOR, CREATED_TIME, CREATED_BY, CREATED_IP, UPDATED_TIME, UPDATED_BY, UPDATED_IP, IS_PRIMARY, TMT, SD, FILE_SK, ID_STATUS_AKHIR_JABAT, KETERANGAN_AKHIR_JABAT, IS_CALON, IS_ACTIVE, IS_DELETED, IS_CURRENT, is_wl, tahun_wl) "
                . "(select pnj.ID_PN, ID_JABATAN, DESKRIPSI_JABATAN, ESELON, NAMA_LEMBAGA, LEMBAGA, UNIT_KERJA, SUB_UNIT_KERJA, ALAMAT_KANTOR, EMAIL_KANTOR, '".$now."' AS CREATED_TIME, 'system2018' AS CREATED_BY, pnj.CREATED_IP, '".$now."' AS UPDATED_TIME, 'system2018' AS UPDATED_BY, pnj.UPDATED_IP, IS_PRIMARY, TMT, SD, FILE_SK, "
                . "CASE
                        WHEN (ID_STATUS_AKHIR_JABAT = '10' AND pn.IS_APLIKASI = '1') THEN ID_STATUS_AKHIR_JABAT = '0'
                        WHEN (ID_STATUS_AKHIR_JABAT = '10' AND pn.IS_APLIKASI IS NULL) THEN ID_STATUS_AKHIR_JABAT = '5'
                        WHEN ID_STATUS_AKHIR_JABAT = '11' THEN ID_STATUS_AKHIR_JABAT = '5'
                        ELSE ID_STATUS_AKHIR_JABAT END AS ID_STATUS_AKHIR_JABAT, KETERANGAN_AKHIR_JABAT, IS_CALON, pnj.IS_ACTIVE, IS_DELETED, IS_CURRENT, is_wl, '".$thndpn."' AS tahun_wl "
                . " from t_pn pn"
                . " left join t_pn_jabatan pnj ON pnj.ID_PN = pn.ID_PN "
                . "where pn.IS_ACTIVE = '1' "
                . "and pnj.IS_ACTIVE = '1' "
                . "and is_wl = '1'"
                . "and IS_DELETED = '0'"
                . "and IS_CURRENT = '1'"
//                . "and tahun_wl = '$curryear'"
                . "and tahun_wl = '2017'"
//                . "and ID_STATUS_AKHIR_JABAT IN (0, 5)"
                . "and ID_STATUS_AKHIR_JABAT <> '1')";
//                . "and pnj.ID_PN = '204865')";
...
//        return FALSE;
    ...
    function do_nonact_pn_wl($idjpn,$idjab) {
        $this->load->model('mglobal', '', TRUE);
        $alasan_non_wl = $this->input->post('alasan_non_wl');
        $data = $this->mglobal->get_data_all('t_pn_jabatan',null,['ID' => $idjab],'tahun_wl');
        $tahun_wl = $data[0]->tahun_wl;
        $cek_data = $this->db->query("SELECT COUNT(id_lhkpn) AS numrows FROM t_lhkpn WHERE id_pn = '".$idjpn."' AND IS_ACTIVE = 1 AND YEAR(tgl_lapor) = '".$tahun_wl."'");
        // if ($cek_data->result()[0]->numrows > 0) {
            // $cek_status = $this->db->query("SELECT IS_ACTIVE, STATUS, ENTRY_VIA FROM t_lhkpn WHERE id_pn = '".$idjpn."' AND IS_ACTIVE = 1 AND YEAR(tgl_lapor) = '".$tahun_wl."'");
            // if ($cek_status->result()[0]->IS_ACTIVE == 1 && $cek_status->result()[0]->STATUS <> 0 && $cek_status->result()[0]->ENTRY_VIA <> 2) {
            //     $res = '9';
            // } else {
                $this->db->trans_begin();
                $arr_pn = array(
                ...
                $arr_jpn = array(
                    'IS_WL' => 99,
                    'ALASAN_NON_WL' => $alasan_non_wl
                );
                $this->mglobal->update_data_('T_PN', $arr_pn, 'ID_PN', $idjpn);
                $this->mglobal->update_data_('t_pn_jabatan', $arr_jpn, 'ID', $idjab);
                //display($this->db->last_query());
                if ($this->db->trans_status() === FALSE) {
                    $this->db->trans_rollback();
                } else {
                ...
                if ($res >= 1) {
                    ...
                    ng::logActivity('Me Non Wajib Laporkan, ID PN = ' . $idjpn);
                }
                // echo intval($this->db->trans_status());
            // }
        // } 
    ...
    public function undeletepn_wl_non($id, $idjpn = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('munitkerja', '', TRUE);
        $this->load->model('mjabatan', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
    ...
    function do_act_non_pn_wl_non($idjpn) {
        $this->load->model('mglobal', '', TRUE);
    ...
    public function editpn_daftarnonaktif($iscln = NULL, $status = NULL, $id = NULL , $idj = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mjabatan', '', TRUE);
    ...
    public function do_act_pn($id_pn, $idjpn) {
        ...
        $data_pn = $this->mglobal->get_data_by_id('t_pn', 'ID_PN', $id_pn);
        $data_pn_jab = $this->mglobal->get_by_id('t_pn_jabatan', 'ID', $idjpn);
        /* update t_pn
         */
        $id_jabatan = $this->input->post('JABATAN');
        // JABATAN
        ...
        if ($data_pn) {
            ...
            if ($asd == "") {
                ...
                $save = $this->mglobal->insert('T_USER', $userBaru);
            }
        }
        ...
        if ($NIK_BARU == "") {
            $arr_pn = array(
                ...
                'TEMPAT_LAHIR' => $TEMPAT_LAHIR,
                //                'JNS_KEL' => $JNS_KEL,
                'JNS_KEL' => $data_pn[0]->JNS_KEL,
                //                'TGL_LAHIR' => date('Y-m-d', strtotime(str_replace('/', '-', $TGL_LAHIR))),
                'TGL_LAHIR' => $data_pn[0]->TGL_LAHIR,
                'EMAIL' => $EMAIL,
                'NO_HP' => $NO_HP,
                'UPDATED_TIME' => time(),
                'UPDATED_BY' => 'admin', //$this->session->userdata('USR'),
            ...
            if($cek_nik){
                ...
                $sql_get_lhkpn = "SELECT * FROM t_lhkpn WHERE ID_PN = '.$id_pn.'";
                $act_get_lhkpn = $this->db->query($sql_get_lhkpn)->result();
        ...
        $this->mglobal->update_data_('T_USER', $arr_user_inst, 'USERNAME', $NIK);
        ...
        //  ---------- validasi ID JABATAN double ----------
        $where_cek_id_jabatan = ['ID_PN' => $id_pn, 'ID_JABATAN' => $id_jabatan, 'ID_STATUS_AKHIR_JABAT' => '1', 'IS_CALON' => '0', 'IS_ACTIVE' => '1', 'IS_DELETED' => '0', 'IS_CURRENT' => '1'];
        $cek_rangkap = $this->mglobal->get_data_all('T_PN_JABATAN', NULL, $where_cek_id_jabatan, 'ID');
        if ($cek_rangkap) {
            $obj = [];
            $non_active = array(
        ...
        $jabatanBaru = array(
            ...
            'IS_CURRENT' => '1',
            'IS_WL' => 1,
            'tahun_wl' => $data_pn_jab->tahun_wl,
            'tgl_kirim' => $data_pn_jab->tgl_kirim,
            'status_lapor' => $data_pn_jab->status_lapor,
            'ID_STATUS_AKHIR_JABAT' => '11',
            'CREATED_TIME' => time(),
            'CREATED_BY' => $this->session->userdata('USR'),
            'CREATED_IP' => $_SERVER["REMOTE_ADDR"],
        ...
        if ($res >= 1) {
            ...
            ng::logActivity('Mengonlinekan PN , ID PN = ' . $id_pn);
        }
        echo intval($this->db->trans_status());
        exit;
    }
    ...
    public function detailpn_penambahan($id, $idjb = NULL, $calon = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('munitkerja', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
	...
	c.UK_ID,c.UK_NAMA,b.INST_SATKERKD,b.INST_NAMA,d.ID_JABATAN,d.NAMA_JABATAN";
        ...
        $data_form = $data;
        $namaform = 'detail_penambahan';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }

    public function detailpn_perubahan($id, $idjb = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('munitkerja', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
...
//                return false;
	...
	c.UK_ID,c.UK_NAMA,b.INST_SATKERKD,b.INST_NAMA,d.ID_JABATAN,d.NAMA_JABATAN";
        ...
        $data_form = $data;
        $namaform = 'detailperubahan';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);

        //$data['form_add'] = $data['html_form_view'];


        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }

    public function detailpn_wl($id, $idjb = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('munitkerja', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
    ...
    public function detailpn_nonwl($id, $idjb = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('munitkerja', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
...
//                return false;
	...
	c.UK_ID,c.UK_NAMA,b.INST_SATKERKD,b.INST_NAMA,d.ID_JABATAN,d.NAMA_JABATAN";
        ...
        $data_form = $data;
        $namaform = 'detailnonwl';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }

    public function detailpn_vertambah($id, $idjb = NULL, $aktif = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('munitkerja', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mglobal', '', TRUE);

        $item = $this->mglobal->get_data_all('t_pn_upload_xls_temp a', '', ['a.ID' => $id], '');
        $detail = $this->mglobal->get_data_all('t_pn_upload_xls_temp a', '', ['a.ID' => $id], '');
        $riwayat_jabatan = $this->mglobal->get_data_all('t_pn_upload_xls_temp a', NULL, ['a.ID' => $id], NULL);

        $data = array(
            'riwayat_jabatan' => $riwayat_jabatan,
            'IS_KPK' => $this->IS_KPK,
            'form' => 'detail_vertambah',
            'idjb' => $idjb,
            'item' => $item[0],
            'buttonaktif' => $aktif,
            'agama' => $this->mglobal->get_data_all('M_AGAMA', null, ['ID_AGAMA' => $item[0]->ID_AGAMA]),
        ...
        //echo $riwayat_jabatan;exit;
        //var_dump($riwayat_jabatan);

        $data_form = $data;
        $namaform = 'detail_vertambah';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }

    public function detailpn_verrubah($id, $idjb = NULL, $aktif) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('munitkerja', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
        ...
        $detail = $this->mglobal->get_detail_pn($id, $idjb);
        $riwayat_jabatan = $this->mglobal->get_riwayat_jabatan($idjb);
        $riwayat_jabatan_baru = $this->mglobal->get_data_all('t_pn_upload_xls_temp a', NULL, ['a.ID' => $idjb], NULL);
        $data = array(
            'riwayat_jabatan' => $riwayat_jabatan,
            'IS_KPK' => $this->IS_KPK,
            'form' => 'detail_verrubah',
            'idjb' => $idjb,
            'item' => $item[0],
            'buttonaktif' => $aktif,
            'agama' => $this->mglobal->get_data_all('M_AGAMA', null, ['ID_AGAMA' => $item[0]->ID_AGAMA]),
            ...
            'penhir' => $this->mglobal->get_data_all('M_PENDIDIKAN', null, ['ID_PENDIDIKAN' => $item[0]->ID_PENDIDIKAN]),
            'dt_detail' => $detail,
            'riwayat_jabatan_baru' => $riwayat_jabatan_baru
        );
        $join = [
            ['table' => 'M_INST_SATKER b', 'on' => 'b.INST_SATKERKD = a.LEMBAGA'],
            ['table' => 'M_UNIT_KERJA c', 'on' => 'c.UK_ID = a.UNIT_KERJA'],
        ...
        //var_dump($riwayat_jabatan_baru);
        $select = "e.*,a.FILE_SK,a.TMT,a.SD,a.ID,a.ID_PN,a.ID_JABATAN,a.DESKRIPSI_JABATAN,a.LEMBAGA,a.UNIT_KERJA,e.ESELON,
	c.UK_ID,c.UK_NAMA,b.INST_SATKERKD,b.INST_NAMA,d.ID_JABATAN,d.NAMA_JABATAN";
        $data['detJabatan'] = $this->mglobal->get_data_all('T_PN_JABATAN a', $join, ['a.ID_PN' => $id], $select);

        $data_form = $data;
        $namaform = 'detail_verrubah';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }

    public function detailpn_vernonaktif($id, $idjb = NULL) {
        $this->load->model('mpn', '', TRUE);
        $this->load->model('munitkerja', '', TRUE);
        $this->load->model('minstansi', '', TRUE);
        $this->load->model('mglobal', '', TRUE);
        ...
        $detail = $this->mglobal->get_detail_pn($id, $idjb);
        $riwayat_jabatan = $this->mglobal->get_riwayat_jabatan($idjb);
        $riwayat_jabatan_baru = $this->mglobal->get_data_all('t_pn_upload_xls_temp a', NULL, ['a.ID' => $id], NULL);
        $data = array(
            'riwayat_jabatan' => $riwayat_jabatan,
            'IS_KPK' => $this->IS_KPK,
            'form' => 'detail_vernonaktif',
            'idjb' => $idjb,
            'item' => $item[0],
            'agama' => $this->mglobal->get_data_all('M_AGAMA', null, ['ID_AGAMA' => $item[0]->ID_AGAMA]),
            'sttnikah' => $this->mglobal->get_data_all('M_STATUS_NIKAH', null, ['ID_STATUS' => $item[0]->ID_STATUS_NIKAH]),
	...
	c.UK_ID,c.UK_NAMA,b.INST_SATKERKD,b.INST_NAMA,d.ID_JABATAN,d.NAMA_JABATAN";
        ...
        $data_form = $data;
        $namaform = 'detail_vernonaktif';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }

    public function rollbacktodraft($ID_LHKPN) {
        $data = array(
            'form' => 'rollbacktodraft',
            'item' => $this->mglobal->get_data_all('T_LHKPN', NULL, NULL, '*', "ID_LHKPN = '$ID_LHKPN'")[0],
        );
        $data_form = $data;
        $namaform = 'rollbacktodraft';
        $data['html_form_view'] = $this->load->view(strtolower(__CLASS__) . '/all_pn_form/form_' . $namaform, $data_form, TRUE);
        $this->load->view(strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_form', $data);
    }
    ...
    public function SettoDraft(){
                            ...
                            <tbody class="body-table">
                                            ...
                                            <tr>
                                                <td><ol type='.'a'.'>
                                                    ...
                                                    <li>Lembar Penyerahan LHKPN yang telah dikirimkan melalui email statistik@kpk.go.id tanggal '. tgl_format($datapn->tgl_kirim_final) .' ke alamat email '. $datapn->EMAIL .' dinyatakan <b>Tidak Berlaku Lagi.</b></li></ol></td>
                                            </tr>
                                        </tbody>
                                    </table>
        ...
        $this->mglobal->update_data_('T_LHKPN', $status, 'ID_LHKPN', $ID_LHKPN);
        $data = array(
...
//////////////////////////////////////////DEVELOPE TAHUN 2018////////////////////////////////////////////////
    public function tracking_pn_wl($offset = 0, $cetak = false){
      ...
      //        display($this->session->userdata('IS_UK'));
              ...
              // prepare paging
              $this->base_url = 'index.php/ereg/all_pn/tracking_pn_wl/';
              $this->uri_segment = 4;
              $this->offset = $this->uri->segment($this->uri_segment);
              //IBO Update
              $this->load->model('Mmpnwn');
...
//               $min_tahun = date('Y');
      ...
      //echo strtolower(__CLASS__) . '/' . strtolower(__CLASS__) . '_' . strtolower(__FUNCTION__);exit;
              if (@$this->iscetak) {
                   ...
                   * @see all_pn/all_pn_index.php
                   */
                  $this->load->view('all_pn/all_pn_tracking_pn_wl', $data);
              }
    }

    public function load_page_tracking_pn_wl() {
        $data_cari = $this->input->get('CARI');
        $this->load->model('Mmpnwn');
        $dtable_output = []; 

        if (empty($data_cari['NIK']) && empty($data_cari['NAMA']) && empty($data_cari['EMAIL']) && empty($data_cari['TGL_LAHIR'])) {
            $dtable_output = array(
                "iTotalRecords" => 0,
            ...
            list($currentpage, $rowperpage, $keyword, $state_active, $sort) = $this->get_param_load_paging_default();
            $page_mode = $this->input->get('page_mode');
            $response = $this->Mmpnwn->load_page_tracking_pn_wl($this->instansi, $currentpage, $keyword, $rowperpage, $page_mode);
    ...
    public function deleteRangkapJabatan($id) {
        $setUpdate = array(
            'IS_ACTIVE' => 0,
            'IS_CURRENT' => 0,
            'IS_DELETED' => 1,
        ...
        $this->mglobal->update_data_('t_pn_jabatan', $setUpdate, 'ID', $id);

        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
        } else {
            $this->db->trans_commit();
            ng::logActivity('Hapus Rangkap Jabatan, NIK = ' . $this->input->post('NIK', TRUE) . ', ID_PN_JABATAN = ' . $id);
        }
        echo intval($this->db->trans_status());
    }

    /**
     * Detail Penyelenggara Negara (copy WL)
     *
     * @return html detail Penyelenggara Negara
     */
    public function form_salin_wl($id, $idjb = NULL) {
        $this->db->select('PNJ.*,
            PN.NAMA,
            PN.NIK');
        $this->db->select("IF(PNJ.`ID_STATUS_AKHIR_JABAT`=0, 'Online','Offline')AS status_wl");
        $this->db->from('t_pn_jabatan PNJ');
        $this->db->join('T_PN PN', 'PN.ID_PN = PNJ.ID_PN', 'right');
        $this->db->where('PNJ.ID', $idjb);
        $this->db->where('PNJ.ID_PN', $id);
     ...
     * @return boolean
     */
    public function salin_wl($id_pn_jabatan) {
        $pnj = $this->mpn->get_jabatan_by_id($id_pn_jabatan);
        if ($pnj) {
            $data = $pnj;
            unset($data->ID, $data->tgl_kirim, $data->status_lapor);
            ...
            $data->tahun_wl = date('Y');
            $copy = $this->mpn->save_jabatan_pn($data);
