...
$role_yang_diijinkan_instansi = in_array($this->session->userdata('ID_ROLE'), $this->config->item('AKSES_ROLE_PAGE_CALON_PN_WL')['filter_instansi']);
$role_yang_diijinkan_unit_kerja = in_array($this->session->userdata('ID_ROLE'), $this->config->item('AKSES_ROLE_PAGE_CALON_PN_WL')['filter_unit_kerja']);
...
<section class="content-header">
  <div class="panel panel-default">
    <div class="panel-heading"><strong>CALON PN/WL</strong></div>
    </div>
    <?php echo $breadcrumb; ?>
</section>
...
<section class="content">
      ...
      <div class="col-md-12">
        ...
        <form method="post" class='form-horizontal' id="ajaxFormCari" action="<?php echo @$thisPageUrl; ?>">
            <div class="box-body">
              <div class="col-md-5">
                <div class="row">
                   <?php //if ($this->session->userdata('ID_ROLE') == 1 || $this->session->userdata('ID_ROLE') == 2 || $this->session->userdata('ID_ROLE') == 108):
                      ...
                      <div class="form-group">
                          <label class="col-sm-4 control-label">Instansi:</label>
                            ...
                            <?php endif; ?>
                            <?php //if ($this->session->userdata('ID_ROLE') == 1 || $this->session->userdata('ID_ROLE') == 2 || $this->session->userdata('ID_ROLE') == 3 || $this->session->userdata('ID_ROLE') == 108):
    ...
    ?>&nbsp;&nbsp;&nbsp;
                                            <?php
                                                if ($item->ID_USER) {
                                                    ?>
                                                <button type="button" class=class="btn btn-sm btn-primary btn-detail" style="background-color: gray; color: white;" href="index.php/ereg/all_pn/reset_password/<?php echo substr(md5($item->ID_USER), 5, 8); ?>" title="Reset Password"><i class="fa fa-reddit-square"></i></button>
                                                <!--<a href="index.php/ereg/all_pn/reset_password/<?php echo substr(md5($item->ID_USER), 5, 8); ?>" class="btn-reset" id="reset_password">Reset Password</a>-->
        <?php
    } else {
        echo '';
    ...
    ?>
                                            <?php if ($this->makses->is_write) { ?>
                                                <!-- <button type="button" class="btn btn-sm btn-primary btn-edit" href="index.php/ereg/all_pn/editpn/<?php echo $item->ID_PN; ?>" title="Edit"><i class="fa fa-pencil"></i></button> -->
                                                <?php if ($havelhkpn[$item->ID_PN] !== true) { ?>
                                                    <!-- <button type="button" class="btn btn-sm btn-default btn-delete" style="background-color: gray; color: white;" href="index.php/ereg/all_pn/reset_password/<?php echo substr(md5($item->ID_USER), 5, 8); ?>" title="Delete"><i class="fa fa-trash" style="color:red;"></i></button> -->
                                                    <button type="button" class="btn btn-sm btn-warning btn-aktif" href="index.php/ereg/all_pn/update_calon_pn/<?php echo $item->ID_PN; ?>" title="Aktifasi"><i class="fa fa-adjust" ></i></button>
                                                    <?php
                                                }
                                            }
                                            ?>
...
//            initiateSelect2CariUnitKerja('1081');
    ...
    var btnEditOnClick = function(self) {
        ...
        $.post(url, function(html) {
            OpenModalBox('Edit Calon Penyelenggara Negara', html, '', 'large');
        });
        return false;
    };
    var btnDeleteOnClick = function(self) {
        ...
        $('#loader_area').show();
        $.post(url, function(html) {
            OpenModalBox('Hapus Data Calon PN/WL Individual', html, '','standart');
        });
        return false;
    };
    var tblDaftarIndividual = {
        ...
        conf: {
            ...
            "responsive": true,
            "bAutoWidth": false,
            "sAjaxSource": "<?php echo base_url('ereg/all_pn/calonpn_showdata/6'); ?>",
            ...
            "aoColumns": [
                ...
                {"mDataProp": "NAMA", bSearchable: true},
                {
                    "mDataProp": function(source, type, val) {
                        ...
                        if (isObjectAttributeExists(source, 'SUK_NAMA') && !isEmpty(source.SUB_UNIT_KERJA)) {
                ...
                },
                {
                    "mDataProp": function(source, type, val) {
                        ...
                        var btnEdit = '<button type="button" class="btn btn-sm btn-success btn-edit" href="index.php/ereg/all_pn/editpn_daftar/1/daftarindividu/' + source.ID_PN + '" title="Edit" onclick="btnEditOnClick(this);"><i class="fa fa-pencil"></i></button>';
                        ...
                        var btnInfo = '<button type="button" class="btn btn-sm btn-info btn-detail" href="index.php/ereg/all_pn/detailpn/' + source.ID_PN + '/' + source.ID + '/' + source.TAHUN_WL + '" title="Preview" onclick="onButton.detail.click(this);"><i class="fa fa-search-plus"></i></button>';
                        var disable = '', btnApprove = '';
                        if (source.IS_ACTIVE == 1) {
                            disable = 'disabled';
                        }
                        return (btnInfo+ " " +btnEdit + " " + btnHapus).toString();
                    },
                    bSortable: false,
                    bSearchable: false
                }
