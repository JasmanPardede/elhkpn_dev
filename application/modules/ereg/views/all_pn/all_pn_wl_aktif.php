...
$role_yang_diijinkan_instansi = in_array($this->session->userdata('ID_ROLE'), $this->config->item('AKSES_ROLE_PAGE_WAJIB_LAPOR')['filter_instansi']);
$role_yang_diijinkan_unit_kerja = in_array($this->session->userdata('ID_ROLE'), $this->config->item('AKSES_ROLE_PAGE_WAJIB_LAPOR')['filter_unit_kerja']);

?>

<script type="text/javascript">
    var dfcthn = <?php echo $default_cari_tahun; ?>;
</script>

<section class="content-header">
  <div class="panel panel-default">
    <div class="panel-heading"><strong>WAJIB LAPOR</strong></div>
    </div>
    <?php echo $breadcrumb; ?>
</section>
                ...
                <div class="box-body">
                    <div class="col-md-5">
                        <div class="row">
                            <div class="form-group">
                                ...
                                <div  class="col-sm-6">
                                    <input type='text' class="input-sm select2 form-control" name='CARI[IS_WL]' style="border:none;padding:6px 0px;" id='CARI_IS_WL' value='<?php echo @$CARI['IS_WL']; ?>' placeholder="ALL">
                                </div>
                            </div>
                    ...
                    <div class="col-md-5">
                        <div class="row">
                            ...
                            <?php //if ($this->session->userdata('ID_ROLE') == 1 || $this->session->userdata('ID_ROLE') == 2 || $this->session->userdata('ID_ROLE') == 3 || $this->session->userdata('ID_ROLE') == 7 || $this->session->userdata('ID_ROLE') == 10 || $this->session->userdata('ID_ROLE') == 13 || $this->session->userdata('ID_ROLE') == 14 || $this->session->userdata('ID_ROLE') == 18 || $this->session->userdata('ID_ROLE') == 31 || ($this->session->userdata('ID_ROLE') == 4 && $this->session->userdata('IS_UK') == 0 && $this->session->userdata('INST_SATKERKD') != NULL) || $this->session->userdata('ID_ROLE') == 108):
                                if ($role_yang_diijinkan_unit_kerja || ($this->session->userdata('ID_ROLE') == 4 && $this->session->userdata('IS_UK') == 0 && $this->session->userdata('INST_SATKERKD') != NULL)): ?>
        ...
        <div class="col-md-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <?php if ($this->session->userdata('USR') == 'admin_kpk') { ?>
                        <input type="hidden" name="T_TAHUN_WL" id="T_TAHUN_WL" value="<?php echo $item; ?>">
                        <button disabled="" class="btn btn-sm btn-primary btn-add"  id="btn-add" href="index.php/ereg/all_pn/generate_wl"><i class="fa fa-plus"></i> Generate PN/WL Tahun Depan</button>
                        <?php
                    } else {
                        echo '<br/>';
                    }
