...
class Mt_pn_upload_xls_temp extends CI_Model {
    ...
    function save_penambahan_jab_pnwl_ver($id, $id_pn) {
        ...
        $ip = $_SERVER["REMOTE_ADDR"];
        $query = "
                INSERT INTO t_pn_jabatan (ID_PN, DESKRIPSI_JABATAN, ID_JABATAN, ESELON, LEMBAGA, NAMA_LEMBAGA, UNIT_KERJA, SUB_UNIT_KERJA, IS_ACTIVE, CREATED_TIME, CREATED_BY, CREATED_IP,IS_WL) SELECT
                        $id_pn,
                        NAMA_JABATAN,
                        ...
                        NAMA_LEMBAGA,
                        NAMA_UNIT_KERJA,
                        NAMA_SUB_UNIT_KERJA, 1, $time, '$usr', '$ip',1
                FROM
                        t_pn_upload_xls_temp
                WHERE
                        ID = $id";
...
//        echo $this->db->last_query();exit();
	...
	function get_ver_pnwl_nonact_two_join(){
		$stringjoin = '((((((`t_pn` `a` JOIN `t_pn_jabatan` `b` ON((`a`.`ID_PN` = `b`.`ID_PN`))) 
			JOIN `m_jabatan` `c` ON((`c`.`ID_JABATAN` = `b`.`ID_JABATAN`))) 
			LEFT JOIN `m_sub_unit_kerja` `d` ON((`d`.`SUK_ID` = `c`.`SUK_ID`))) 
			LEFT JOIN `m_unit_kerja` `e` ON((`e`.`UK_ID` = `c`.`UK_ID`))) 
			LEFT JOIN `m_inst_satker` `f` ON((`f`.`INST_SATKERKD` = `e`.`UK_LEMBAGA_ID`))))';
	...
	function get_ver_pnwl_nonact_two_condition($pqry){
		$stringcondition = ' a.NIK NOT IN (SELECT DISTINCT(NIK) FROM t_pn_upload_xls_temp WHERE $qryins) AND ID_STATUS_AKHIR_JABAT = 0 AND IS_CALON = 0 AND IS_DELETED = 0 AND a.IS_ACTIVE = 1 AND b.IS_CURRENT = 1
		$pqry AND (SELECT COUNT(NIK) FROM t_pn_upload_xls_temp) <> '' ';
        
		if (!is_null($cari) && $cari != '') {
			$stringcondition .= ' AND (`a`.NIK LIKE CONCAT('%', '" . $cari . "' , '%') OR `a`.NAMA LIKE CONCAT('%', '" . $cari . "' , '%') OR `c`.NAMA_JABATAN LIKE CONCAT('%', '" . $cari . "' , '%'))';
		}
		$stringcondition = ' ORDER BY `a`.`NIK`';
		if($limit_mode){
			$stringcondition .= ' LIMIT $rowperpage ';
        }
		return $stringcondition;
	}
	
	function count_ver_pnwl_nonact_two(){
		$sql_select = 'count(*) as jml ';
		 $sql_join = $this->get_ver_pnwl_nonact_two_join();
		 $sql_condition = $this->get_ver_pnwl_nonact_two_condition();
		 
		 $sql_query = "select " . $sql_select." from table ".$sql_join."  where ".$sql_condition;
    ...
    function get_ver_pnwl_nonact_two($ins, $offset = 0, $cari = NULL, $rowperpage = 10, $limit_mode = TRUE) {
        $is_uk = $this->session->userdata('IS_UK');
        $id_uk = $this->session->userdata('UK_ID');
        ...
        if (!is_null($ins) && $ins && $this->role > 2)
		 ...
		 $sql_select = '`a`.ID_PN, b.ID AS ID_PN_JAB, 
		`a`.`NIK` AS `NIK`,
		`a`.`GELAR_DEPAN` AS `GELAR_DEPAN`,
		`a`.`NAMA` AS `NAMA`,
		`a`.`GELAR_BELAKANG` AS `GELAR_BELAKANG`,
		...
		`a`.`EMAIL` AS `EMAIL`';
		 ...
		 $sql_join = $this->get_ver_pnwl_nonact_two_join();
		 $sql_condition = $this->get_ver_pnwl_nonact_two_condition($pqry);
		 
		 $sql_query = "select " . $sql_select." from ".$sql_join."  where ".$sql_condition;
