<?php $primary_key_secure = $primary_key . "_secure"; ?>
<form role="form" id="ajaxFormEdit" action="index.php/efill/validasi_penerimaan_kas/update_perubahan" >
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">FORM DATA PENERIMAAN KAS</h4>
    </div>
    <div class="modal-body row">
        <ul class="nav nav-tabs" role="tablist">
            <?php
            $tot2 = 0;
            $sub2 = '0';
            $label = array('A', 'B', 'C');
            $golongan = $golongan_penerimaan_kas_pn;
            for ($i = 0; $i < count($golongan); $i++) :
                ?>
                <li class="<?= ($i == 0) ? 'active' : ''; ?>">
                    <a class="<?php echo $label[$i] . '_PENERIMAAN'; ?>" href="#tabs<?= $label[$i] ?>" data-toggle="tab" onClick="change('<?= $label[$i] ?>')"><?= $label[$i] ?>. <?= $golongan[$i] ?></a>
                </li>
            <?php endfor; ?>
        </ul>
        <?php
        $jenis = $jenis_penerimaan_kas_pn;
        $lain = FALSE;

        $jAPN = 0;
        $jAP = 0;
        $JB = 0;
        $jC = 0;
        ?>
        <div style="width: 100%;">
            <input type='hidden' name='id_imp_xl_lhkpn_penerimaan_kas' id='id_imp_xl_lhkpn_penerimaan_kas'  value='<?php echo $item->id_imp_xl_lhkpn_penerimaan_kas_secure; ?>'>
            <div class="tab-content">
                <?php
                for ($i = 0; $i < count($jenis); $i++) :
                    ?>
                    <div class="tab-pane <?= ($i == 0) ? 'active' : ''; ?>" id="tabs<?= $label[$i] ?>">
                        <table class="table table-bordered table-hover">
                            <thead class="table-header">
                                <tr>
                                    <th width="40px">NO</th>
                                    <th>JENIS PENERIMAAN</th>
                                    <?php
                                    $style = 'style="width: 150px;"';
                                    ?>
                                    <?php echo ($i == 0) ? '<th ' . $style . '>PENYELENGGARA NEGARA (Rp.)</th>' : '<th>ISIAN NILAI PENERIMAAN KAS (Rp.)</th>'; ?>
                                    <?php echo ($i == 0) ? '<th ' . $style . '>PASANGAN (Rp.)</th>' : ''; ?>
                                </tr>
                            </thead>
                            <tbody class="tabs<?= $label[$i] ?>">
                                <?php
                                for ($j = 0; $j < count($jenis[$i]); $j++) :
                                    $PA_val = 'PA' . $j;
                                    ?>
                                    <tr>
                                        <td><center><?= ($j + 1) ?>.</center></td>
                                <td><?= $jenis[$i][$j]; ?></td>
                                <td align="right">
                                    <?php
                                    /**
                                     * lws
                                     * Penerimaan PN
                                     */
                                    ?>
                                    <?php $code = $label[$i] . $j; ?>
                                    <input type="text" name="<?php echo $code; ?>" id="<?php echo $code; ?>" class="form-control money" value="<?php echo !is_null($item->$code) && $item->$code != '' ? $item->$code : '0'; ?>" />
                                </td>
                                <?php if ($i == 0) : ?>
                                    <?php
                                    /**
                                     * lws
                                     * Penerimaan Pasangan
                                     */
                                    ?>
                                    <td align="right">
                                        <input type="text" name="<?php echo $PA_val; ?>" id="<?php echo $PA_val; ?>" class="form-control money" value="<?php echo !is_null($item->$PA_val) && $item->$PA_val!='' ? $item->$PA_val : '0'; ?>" />
                                    </td>
                                <?php endif; ?>
                                </tr>
                                <?php
                            endfor;
                            ?>
                            </tbody>
                        </table>
                    </div>
                    <?php
                endfor;
                ?>
            </div>
        </div>
        <div class="clearfix"></div>
        <div class="modal-footer">
            <button type="submit" class="btn btn-primary btn-sm" ><i class="fa fa-save"></i> Simpan</button>
            <button type="button" class="btn btn-danger btn-sm" data-dismiss="modal"><i class="fa fa-remove"></i> Batal</button>
        </div>
    </div>
</form>

<script type="text/javascript">
    $(document).ready(function () {
//
//        var list_jenis_harta = load_html('portal/data_harta/get_jenis_pelepasan_with_data', '<?php echo $item->JENIS_PELEPASAN_HARTA; ?>');
//        $('#JENIS_PELEPASAN_HARTA').html(list_jenis_harta);
//


        var url = location.href.split('#')[1];
        url = url.split('?')[0] + '?upperli=li4&bottomli=0';
        ng.formProcess($("#ajaxFormEdit"), 'update', url);
    });
</script>