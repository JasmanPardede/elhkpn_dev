<?php
    switch ($form) {
        ...
        foreach ($data as $item) {
            ...
            $NAMA_KOORD_ENTRI = $item->NAMA_KOORD_ENTRI;
...
<div>
    ...
    <p style="font-size:10px;  text-indent: 3em; ">
        Serah Terima Dokumen LHKPN dari Koord. Customer Service Kepada Koord Data Entry, sebanyak <?php echo $jml;?> Dokumen atas nama penyelesaian seperti tersebut dibawah ini :
    </p>
    <?php foreach($dataItem as $key => $row): ?>
        <h3><?php echo $key ?></h3>
        <table width="100%" class="tb-1" border="1" style="font-size: 8px; margin-bottom: 15px;">
            <tr>
                <th width="1%">No</th>
            ...
            <?php $i = 0; foreach($row['detail'] as $item): ?>
                <tr>
                    ...
                    <td>
                        <?php echo $item->NAMA_JABATAN.' - '.$item->DESKRIPSI_JABATAN; ?>
                    </td>
                    <td>
