...
Class Mongo_db
{
	...
	private function config(array $manual_config = []): self
	{
		...
		$file_path_config = NEWPATH.'config'.DIRECTORY_SEPARATOR.self::CONFIG_FILE_NAME.EXT;

		if (!file_exists($file_path_config)) {
			$this->error('Make sure config file exists and all parameters are valid.');
		}

		$file_config = include $file_path_config;
		$file_config = $file_config[self::CONFIG_ARRAY_NAME];
