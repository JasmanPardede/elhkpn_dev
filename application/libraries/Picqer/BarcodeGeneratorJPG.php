...
class BarcodeGeneratorJPG extends BarcodeGenerator {
     ...
     * @public
     */
    public function getBarcode($code, $type, $widthFactor = 2, $totalHeight = 30, $color = array(0, 0, 0), $filename = "", $jpeg_quality=95) {
        ...
        $this->resImgWidth = $width;
        $this->resImgHeight = $height;
        ...
        $image = TRUE;
        if ($filename !== "") {
            imagejpeg($jpg, $filename, $jpeg_quality);
            return TRUE;
        } else {
            ob_start();
            if ($imagick && isset($imageMagickObject)) {
